...
> **Resolution status — HIGH-tier MISSING_BOUNDS_CHECK partially verified 2026-07-02.** This is the real P3 target (~174 leads); earlier "P3 HIGH bounds" blocks had drifted into the MEDIUM mirror. Same bugs, different line numbers (`ArmyData:1029` here vs `:1027` MEDIUM). Verification pass in progress — the gameobj/gfx/AI index cluster is done; the string-overflow, file-parse, and supervised/deferred buckets remain.
>
> **Fixed this pass (commits ccd05c04, 5d355062, e56fd86d, 88fabfc0, d9aba273, + installation/installationdata):** `MaterialPool`/`MessagePool` player derefs → `safe_player`; `tileset` load bounds `GetTileNum()`; `UnitActor` AddVision/RemoveVision/GetIDAndType guard `m_playerNum`/`owner`; `SelItem::ConstructPath` empty-`m_waypoints` guard; `ArmyData::Battle` empty-army guard; `CityData` ring `-1` OOB write; `gameinit` scenario-player upper bound; `SettleMap::HandleCityGrowth` citydata/sizeIndex; `tiledraw` build-queue DB `Get()` null-checks; `installation`/`installationdata` owner via `safe_player`.
>
> **Already fixed / false-positive (verified in code):** raw `g_player[` is essentially gone (migrated to `player_Get`/`safe_player`); `FeatTracker` (safe_player + `type` bound at AddFeat), `DiplomaticRequest` (safe_player), `messagedata`, `Happy::CalcHappiness` (null-guarded), `thronedb::GetThroneInfo`, `mapanalysis` empire accessors, `scheduler` goal_type, `ArmyData` empty-army (1029/1062/1068/1413), `Order:252`, `agreementmatrix:140`, `appstrings:41`, `profileDB:551`. False-positives: `terrainutil` `GetDBImprovement` self-bounds (returns -1); `gameinit` `ai_players[]` (k_MAX_PLAYERS slots → k_MAX_PLAYERS array, no overflow); `director:1094` player deref sits under `#ifdef _PLAYTEST` (disabled in normal builds). Absent: `C3Player.cpp`, `Cont.cpp`, `GWCivRecord.cpp`, `C3GameState.cpp`.
>
> **Remaining (not yet done):** string-overflow class (P4-flavored: `c3cmdline:2506/3078`, `c3errors:51/169`, `civ3_main:326/398`, `slicif`); file-parse bounds (supervised: `spritefile` ×~18, `aui_image`, `MapFile`, `targautils`, `tracklen`, `effectspritegroup`/`goodspritegroup` frame-count loops, `iparser`, `SlicRecord:99`); supervised (`diplomat` m_foreigners, `DiplomaticRequestData`, `WrlEnv`, `DB.cpp` template `Access`); `C3Science` memcpy API; `net_*` deferred per ground rules.
...
> **Resolution status — HIGH-tier NULL_DEREFERENCE partially verified 2026-07-03.** ~131 leads; earlier "P3 null" blocks had drifted into the MEDIUM mirror. Verification pass in progress — like the bounds section, the overwhelming majority are **already fixed** (the `g_player[X]->` → `player_Get`/`safe_player` migration added index+null guards to most sites) or stale.
>
> **Fixed this pass (commits 85c143d5, 5e9f4304, d23df003):** `TradeOffer::RemoveAllReferences` and `TaxRate::SetTaxRates` bare `player_Get` → `safe_player`; `Agent::FindPath` (both overloads) null-checks `GetOrder(0)`; `ArmyEvent` combat handler guards `player_Get(attack_owner/defense_owner)->CreateLeader()` and `player_Get(defender->GetOwner())->m_builtWonders`; `Player::ExchangeMap` guards `player_Get(recipient)`; `Slic_CreateUnit::Call` returns `SFN_ERROR_DEAD_PLAYER` before dereferencing a dead owner.
>
> **Verified already-fixed:** `AgreementData` (safe_player), `ArmyData` empty-army (guarded), `FeatTracker` (safe_player), `Happy::CalcHappiness` (null-guard at entry), `MaterialPool`/`MessagePool` (fixed earlier), `MessageData` SetTitle/SetMsgText/text (now `std::string`, ternary-guarded), `Order::operator delete` (`if(!ptr) return`), `Unit::KillUnit`/`RemoveAllReferences` + `UnitData::SetPos` + `Player::GiveAdvance` (index+null guards), `PlayerEvent` handlers (safe_player ×29), `Regard::GetUpdatedRegard` (null-guard), `SlicSymbol::GetPos`/`GetPlayer` (m_struct guard), `UnitActor:1933/384` + `directorevent:222` + `civ3_main:1092` (fixed earlier). `aui_image:323` sits behind `assert(0)` (dead diagnostic path).
>
> **Spot-check list resolved 2026-07-03 (commit 1b97e703):** Fixed the genuine ones — `Slic_StringCompare` (null `GetNameStr` strings), `UnitActor:1546` (bare re-`UnitArmy()`), `Goal:2487` (bare `player_Get(target_owner)->m_builtWonders`), `aui_tipwindow:87` (`m_staticTip`). Already-fixed/stale: `Player` alliance (Form/Break guard `player_Get(ally)`), `SlicFunc:7397` Pillage (`player_Get(pl)` guarded above; divisor already `== 0`-checked), `Regard:79`. **Structurally-safe, left as-is (invariant: a live army/city has a live owner; hot paths):** `ArmyData:2668/2564` and the `CityData` `player_Get(m_owner)->GetGovernmentType()` family (~10 sites). **Misframed (GetCell never returns NULL for an in-range pos — the real risk would be OOB, not null):** `c3cmdline:3168` (mouse-tile pos), `CityInfluenceIterator:234` (city center). **Supervised remainder:** `Goal` `m_target_city.GetCityData()` cluster (assert-only, ~7 sites — cache+early-return refactor), `SlicSymbol:174/236/313` (`m_struct` deref in typed-access paths), `background:372`/`c3fancywindow:236` (UI ldl-object nulls), `Cell:342/984` (DB Get on valid good/terrain indices). Absent: `C3Robot`/`GWArchive`/`GWFile`. Deferred: `net_*`.
...
>
> **Not bugs / stale leads (skipped):** `Barbarians.cpp` `g_rand->Next(x - 1)` leads are covered by `RandomGenerator::Next(sint32)`, which returns 0 for `r <= 0`; `Cont.cpp` and the GameWatch files (`GWArchive.cpp`, `GWCivRecord.cpp`, `GWFile.cpp`) are absent from the current tree.
>
> **Correction (2026-07-02):** an earlier annotation wrongly listed `Installation.cpp` as absent — it exists at `gs/gameobj/installation.cpp` (lowercase) and the lead was **genuinely open**: `player_Get(i)` does NOT upper-bound its index (`return g_player ? g_player[i] : nullptr`), so `if(GetOwner() >= 0 && player_Get(GetOwner()))` read `g_player[GetOwner()]` out of bounds when `GetOwner() >= k_MAX_PLAYERS`. `Installation::RemoveAllReferences` now uses `safe_player(GetOwner())` (bounds-checked) at both sites.
