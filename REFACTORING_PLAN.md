...
| P3 | HIGH-severity findings, category batches | DANGEROUS_SHIFT + DIVISION_BY_ZERO done (1st-party, HIGH tier). MISSING_BOUNDS_CHECK + NULL_DEREFERENCE: many real bugs fixed, but **HIGH-tier sections (~305 leads) still unverified** — prior annotations drifted into the MEDIUM mirror (see tier-drift note) | multi-session |
