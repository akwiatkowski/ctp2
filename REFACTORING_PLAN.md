...
- P11 GPU rendering: **Stage 1 done; Stage 2 A–F — A (32bpp UI blitters) + E (SDL3 default) done 2026-07-14. B (world→32-bit) actively in progress 2026-07-14.** The "blocked on licensed `.SPR` assets" premise was **FALSE** — the game data is on disk (`ctp2_data/default/graphics/`), so B/C/D are unblocked. B is well advanced: the modern-asset converter (Stage 0) is complete and **all ~36 raw-pixel world *tile* writers are converted to a dormant 32-bit path**; sprites + primitives + the surface flip + B1 atlas + C GPU-fog remain. See the P11 section.
