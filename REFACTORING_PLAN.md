...
- **Sanitizer hardening, 2026-09-09 (ADR-012, commit `0ac0b144`):** `Order::m_eventType` and `TerrImproveData` bools init to storable sentinels; `GAME_EVENT`/`UNIT_ORDER_TYPE` range-checked before `static_cast`; headless `--load-game` sizes player slots from the save's alive count. UBSan unit 643/643, 0 findings; 500/500 marathon rounds + deep save/load to 520; tier-C re-run in flight to re-confirm. Oracle-gating (`5d984857`: secondary blits 2320→0, SoftBlit 2489→256) and gallery naval placement (`831a230e`) landed alongside.
...
- **Memory-safety loop 2026-08-26 (/goal): ui/interface message/victory family RAII'd — 15 commits (`14273b99`…`dc2eefa7` + `92d0cca4`…).** creditsscreen, messageeyepoint, messagemodal (**real leak fix**: `~MessageModal` never freed the eye-point helper), hotseatlist, EndgameWindow, messagewin (+~230 lines of `#if 0` CtP1 bodies deleted), victorywin (both `mycleanup` macros gone; non-owning registries → `std::array`), messagelist, spnewgamewindow (69-delete dtor sweep → `unique_ptr` members), messageiconwindow, AttractWindow (+PointerList→vector), citymanager, MapCopyBuffer (jagged grid→flat vector) + scenarioeditor dialogs, text_hasher (bucket chains→unique_ptr), rankingtab/timelinetab/linegraph (**GenrateGraph grid made function-local — fixed timelinetab's scalar-delete-on-array UB en route**). Ratchet: `raw_delete` 1593→1444, `raw_new` 4159→4133, `pointerlist_uses` −9 total. **Pre-existing headless failures attributed at base `4a7645ea`, NOT from this loop** (KB items #35051/#35061): bare `end_turn` sometimes not advancing the round (slice-headless/expansion-headless/load-stress off-by-one), long-game `end_turn 50`→out_of_range, city-capture `not_at_war`.
- **Loop continued 2026-08-26 (afternoon): gs/gameobj advances cluster — 9 more commits (`92d0cca4`…Happy).** messageiconwindow, AttractWindow (+PointerList→vector), citymanager, MapCopyBuffer+scenarioeditor dialogs, text_hasher, rankingtab/timelinetab/linegraph (scalar-delete UB fix), **Advances masks**: CanAskFor/CanOffer → `vector<uint8_t>`, CanResearch → const-ref to m_canResearch (**fixed two scalar-delete-on-array UB sites** in chart.cpp/sci_advancescreen.cpp + a debug-verb leak), DiplomaticRequestData Accept/Reject switches → scoped unique_ptr SlicObjects, **UnitData** cargo/city/mask → unique_ptr (JSON bridge + net_unit/net_city updated; fixed unit-type-change cargo leak; ubsan-smoke verified), Happy tracker → unique_ptr. Tick-only noise passes: unitevent, GoodyHuts, endgame, PlayerEvent, armyevent, CityEvent, Advances, messageactions. Ratchet now **raw_delete 1423, raw_new 4130** (day: −170 delete).
...
| Counter | Count (2026-08-26) | Nature of the remainder |
| --- | --: | --- |
| `raw_new` | 4139 | RAII conversion (P6); ~400 medium files need per-file ownership analysis, no mechanical tier. (Includes ~11 `--new-game`-style string false positives absorbed in test files.) |
| `raw_delete` | 1463 | same P6 effort (paired with `raw_new`). |
| `c_allocation` | 69 | `malloc`/`calloc`/`realloc`/`free`; 2026-07-17 loop drove 101→81 (RadarMap/ProjectFile → `std::vector`, `TifBuffer` RAII for TIFF loaders, dead code deleted). Remainder: slic `PSlicObject` ownership web (slicif/SlicSegment/SlicSymTab/json_save — one coherent mini-project), prjfile `getData_*` API, tileutils' `LowPassFilter` ownership-juggling corner (audit, not wrap). |
| `type_erased_casting` | 2512 | `void*`/C-cast handoffs; opportunistic cleanup only — large, cross-cutting, low crash-relevance. |
...
- [~] **Interactive `Draw` path (main-map enabler)** — the real "use new sprites in play" step.
  - [x] **Non-directional-action fix (critical)** — DONE 2026-07-17. Verified in-engine (headless `render_map`-style capture + a one-shot draw log) that idle units were **still falling back to legacy**: the atlas packs `IDLE` as a single non-directional facing (`facings: 1`), but the unit requests `IDLE` at its heading (`facing 3`), so `FindRect("IDLE", 3, …)` missed. Added `ModernSpriteAtlas::FacingCount(action)`; the unit hooks (`Draw` + `DrawDirect`) now request stored facing 0 for non-directional actions (still mirrored at facing ≥ 5, like `Sprite::DrawDirect`) and fold/mirror only for directional ones. Confirmed the idle unit now draws from the atlas and renders correctly (pixel-inspected). Doctest covers `FacingCount`. **Without this the interactive path was a near-no-op** (idle is the common on-map state).
