...
- P11 GPU rendering: **Stage 1 done; Stage 2 A/B/E done + verified 2026-07-14** (A 32bpp UI blitters, E SDL3 default, **B world→32-bit** — all tile/sprite/primitive writers converted, `m_mapSurface` flipped to ARGB8888, `slice-ui` pixel oracle green across multi-turn gameplay). **B1 modern-first atlas sprite path also landed** (converter Stage 0 → PNG loader → atlas load+Blit → `current` pointer → `UnitSpriteGroup` hook behind `CTP2_MODERN_SPRITES`, default off; slice-ui passes with it on). The "blocked on licensed `.SPR` assets" premise was **FALSE** (data on disk). **D (per-layer GPU compositing) + C (GPU fog-mask) both DONE + verified 2026-07-15** — D: world-only + UI-only GPU textures (gated `CTP2_GPU_LAYERS`, slice-ui green both paths); C: terrain fog as a GPU mask composited over the world texture (gated `CTP2_GPU_FOG`, verified in-game by Olek). Remaining: **F** (smooth camera), + atlas-path polish (draw-flags, scaled/mirrored facings, Good/Effect groups). See the P11 section.
...
| P11 | GPU-path rendering | **Stage 1 ✅.** **Stage 2 A–F:** A (32bpp UI blitters) ✅; E (SDL3 default) ✅; **B (world→32-bit) in progress** — asset "blocker" was false (data on disk); Stage 0 converter ✅ + all ~36 world tile writers converted (dormant 32-bit) ✅; sprites/primitives/flip/B1/C remain. F post-GPU polish. | B active on branch `p11-stage2-gpu-world` |
...
- [ ] **Sub-tile smoothness — buttery GPU pan (ADR-001).** Decided 2026-07-16: oversized GPU world texture (screen + ~2-tile margin) panned on the GPU via a moving source rect; whole-tile `ScrollMap` recenters underneath, margin hides the seam; macOS supplies native momentum via the wheel-event stream. No CPU memmove shortcut. Incremental:
  - [ ] **2a. Oversized world substrate** — world surface + texture at screen+margin; mirror the background window into it at the (margin,margin) offset; present samples the screen-sized centre (identity at `CameraOff=0`). Visible output unchanged (margin is an unshown border). Verify slice-ui green.
  - [ ] **2b. Render the margin** — map renders the screen+margin extent into the oversized surface so the border holds real terrain (relies on `RetargetTileSurface` sizing `m_surfaceRect` from the dest surface).
  - [ ] **2c. Wire the pan** — trackpad accumulate → whole tiles `ScrollMap` into the oversized surface + sub-tile remainder as `CameraOff`; present offset by `CameraOff` (margin now populated → no black edge).
  - [ ] **2d. Ship** — flip `CTP2_GPU_LAYERS`+`CTP2_GPU_CAMERA` default-on for the map, edge rubber-band polish, tune margin, pixel-oracle regression + in-game "finished" check.
- [ ] **Pinch-to-zoom** — SDL3 removed multi-gesture events; needs probing how macOS pinch arrives under SDL3 (wheel+Ctrl? native trackpad magnify? nothing?) before wiring to the existing `AddZoomImpulse` camera path.
...
**Remaining-session tally (2026-07-14, pre-park):** A + E + B + B1 + C + D + F done. Quad path (G) parked (see above). Modern-sprite track now the active P11 line.
