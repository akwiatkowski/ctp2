...
- [ ] **Interactive `Draw` path (main-map enabler)** — add the modern hook to `UnitSpriteGroup::Draw` (and Good/Effect/City `Draw`) so the PRIMARY gameplay map uses the atlas, not just the screenshot/battle/overlay renders. Needs a lock-free blit onto the screenmanager's pre-locked base. **This is the actual "use new sprites in play" step** — do it before the default-on flip. (See reach-correction above.)
- [ ] **City group draw swap** — `CitySpriteGroup` is the last unwired sprite group (cities still draw legacy even with the toggle on; 110 GC atlases already cached). Same pattern as Good/Effect. Discovered 2026-07-17 while scoping default-on.
- [ ] **Outline / feathering on the atlas path** — the modern `Blit` ignores the `outlineColor` selected-unit outline; with modern on, a selected/active unit would lose its coloured outline. Handle (implement, or fall back to legacy only when an outline is requested) before default-on.
