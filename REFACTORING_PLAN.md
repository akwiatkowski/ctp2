...
| P4 | Unsafe string APIs (`unsafe_string_api=57`, first-party) | underway; ratchet re-scoped (libs excluded); 85 real conversions; clean pool exhausted (tail = net/dead/idiom-cascade) | multi-session |
...
`strcpy`/`strcat`/`sprintf` are the buffer-overflow class — higher real-world risk than the raw-`new` count. Ratchet **re-scoped to first-party 2026-07-03** (vendored `libs/` excluded, see P6 snapshot): `unsafe_string_api=92`. The old libs-inclusive counter was 1360, of which **1268 (93%) were vendored anet/freetype** — noise that hid the real first-party surface. First-party is now **65** (was 92 after the re-scope; 2026-07-03 grind converted netshell lobby UI 13, and assorted fixed/exact/source-bounded sites across trademanager/loadsavescreen/textutils/effectspritegroup/civapp/aui_stringtable/battle.h/UIUtils/ns_customlistbox/gameselectwindow/netfunc). The remaining 65 is the hard/deferred tail: `net/` (18, deferred) + `netfunc` offset-writes (4, phase-2); the embedded `GetXName(MBCHAR*)` "fill caller's buffer" idiom (`CivilisationData`/`Player`/`Civilisation`) which cascades to 40+ call sites incl offset-callers (`finalText+strlen`) — off-limits; dead code (`AgreementData` 11, `BuildQueue::Dump` 5); and param-buffer functions needing caller threading (victorywin/infowin/texttable/UnitSpriteGroup/ldl_data + `scenarioeditor::GetLabel`, which has a latent `snprintf(ptr, sizeof(ptr), …)` truncation bug to fix when threaded).
