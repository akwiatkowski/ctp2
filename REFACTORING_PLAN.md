...
**Remaining-session tally (2026-07-14):** A + E done; **B is ~60% done** (Stage 0 converter + all world tile writers converted, dormant). Remaining B: sprite blitters + primitives, then the surface flip + oracle verify. Then B1 (C++ PNG loader on vendored zlib + atlas RGBA draw + modern-first loader) and C (GPU fog-mask blend). Est. **~2 more sessions** with subagent fan-out for the mechanical bulk. **Nothing is asset-gated** — the `.SPR`/tileset data is on disk at `ctp2_data/default/graphics/`.
