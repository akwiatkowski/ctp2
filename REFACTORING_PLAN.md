...
- P11 GPU rendering: **Stage 1 done; Stage 2 has a counted checklist (A–F).** Item E (SDL3 default, ~1–2s) is doable now; the world-32bit core (B/C/D, ~8–13s) is **blocked on Olek supplying local licensed `.SPR` assets** for the M10 atlas — the asset hand-off is the real schedule driver. See the P11 section.
...
| P11 | GPU-path rendering | **Stage 1 ✅ done** (pixel oracle + dirty-rect present, −73% upload bytes). **Stage 2 checklist A–F:** A (32bpp UI blitters) ✅; B/C/D world-32bit + render flags + layer compositing **blocked on licensed `.SPR` assets**; E (SDL3 default, ~1–2s) doable now; F post-GPU polish | staged — E unblocked, B/C/D asset-gated |
...
- [ ] **B. World render pipeline → 32-bit** — `(~3–5s)` **BLOCKED on licensed `.SPR` assets (see M10 B1).** The world still renders in 16-bit RGB565: `k_SHARED_SURFACE_BPP = 16` (`ctp/civ3_main.cpp:216`); the tile blitter writes hardcoded `Pixel16` into `m_mapSurface` (`gfx/tilesys/tiledmap.cpp:588`), which is allocated at the global depth (`aui_Factory::new_Surface` → `c3ui_Get()->BitsPerPixel()`). Going 32-bit needs 32-bit tileset/sprite textures from the M10 atlas first — otherwise it degrades to a per-tile 565→8888 blit that defeats the purpose. Sub-steps:
  - [ ] B1. Consume 32-bit tileset/sprite textures from the M10 atlas (gated on M10 B1 real-asset parity — needs Olek's local CTP2 data path).
  - [ ] B2. Flip `k_SHARED_SURFACE_BPP` 16→32 and make the tile blitter write 32-bit pixels.
  - [ ] B3. Delete the world→secondary 565→8888 conversion; re-assert the pixel oracle over world content.
- [ ] **C. Draw-time render flags as GPU ops** (fog / desaturation) — `(~2–3s)`, depends on **B** (needs the 32-bit world). Currently CPU per-pixel in the software composite.
- [ ] **D. Per-window/per-layer `SDL_Texture` GPU compositing** — `(~3–5s)`, depends on **B**. The full aui rewrite the hybrid present deliberately avoided (see feasibility note above).
- [ ] **E. Phase D — SDL3 as the default backend** — `(~1–2s)`, **asset-optional, independent of B — the only remaining Stage 2 item doable now.** Probe builds + `fast unit` green under `-Dsdl_backend=sdl3`; needs the full integration suite (pixel oracle, slices, determinism) green under SDL3, then the default flip from SDL2.
- [ ] **F. Smooth camera** (momentum scroll, pinch-zoom) — post-GPU polish, depends on **D**. (The line-184 item.)

**Remaining-session tally:** **E (~1–2s) is doable now**; **B+C+D (~8–13s) are gated on Olek supplying local licensed `.SPR` data** (the M10 real-asset atlas is their prerequisite). F is post-GPU polish. So "how many sessions to finish P11" = ~1–2 unblocked + ~8–13 asset-gated, with the asset hand-off being the true schedule driver, not the code volume.
