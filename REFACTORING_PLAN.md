...
- [ ] **Sub-tile smoothness** — the pan is tile-stepped (chunky on slow drags). Options: (a) native `ScrollMapSmooth` (CPU pixel-scroll via `ScrollPixels`, reveals terrain, ships default-on — but it's gated-off/duplicate-defined dead-ish code, so needs revival + a perf check on its per-event edge `RepaintTiles`); (b) GPU sub-tile glide (`CameraOff` for the <1-tile remainder — buttery but only active with the GPU camera/layers stack, so would NOT be smooth on a default build unless that stack goes default-on, which carries the D UI-ghost caveat). Decide after weighing (a)'s revival cost vs (b)'s default-off limitation.
