...
**Recommended next order (2026-07-03):** crash-class work (P1/P3/P4) is now done or at its practical floor, so the highest-value remaining moves are:
1. **P5** — JSON-load derived-cache audit. Bounded (~1 session), well-defined, correctness-class (wrong-state-after-load). Best ROI now.
2. **P7** — close-out docs; small, and lets the original Definition of Done be formally declared (only P7 blocks it).
...
4. **P2** — ASan-on-Linux; blocked only on starting a container engine (`colima start` / Docker), then one build. User action gates it.
5. **P6** — the long multi-session RAII effort (413 medium files); interleaves well but is the least crash-relevant. Do after ASan (P2) lands to catch double-frees.
