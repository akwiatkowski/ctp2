...
- P11 GPU rendering: **Stage 1 done; Stage 2 checklist A–F — A (32bpp UI blitters) + E (SDL3 default backend) done 2026-07-14** (E also fixed a GPU-present regression that black-screened both backends). The remaining world-32bit core (B/C/D, ~8–13s) is **blocked on Olek supplying local licensed `.SPR` assets** for the M10 atlas — with A+E done, the asset hand-off is now the *only* thing gating P11. See the P11 section.
...
| P11 | GPU-path rendering | **Stage 1 ✅ done** (pixel oracle + dirty-rect present, −73% upload bytes). **Stage 2 checklist A–F:** A (32bpp UI blitters) ✅; E (SDL3 default backend) ✅ + fixed a dead-GPU-present regression; B/C/D world-32bit + render flags + layer compositing **blocked on licensed `.SPR` assets**; F post-GPU polish | staged — A+E done, B/C/D asset-gated |
...
**Remaining-session tally:** **A + E done.** All remaining Stage 2 work — **B+C+D (~8–13s) is gated on Olek supplying local licensed `.SPR` data** (the M10 real-asset atlas is their prerequisite); **F is post-GPU polish** (depends on D). So with the only asset-optional items (A, E) complete, "how many sessions to finish P11" is now **entirely gated on the asset hand-off** — no unblocked code work remains until the `.SPR` data path exists.
