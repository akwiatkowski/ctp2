    ...
    m_generic_goals         ()
{
	...
	{
		auto generic_goal = std::make_unique<Goal>();

		generic_goal->Set_Type(i);

		m_generic_goals.push_back(std::move(generic_goal));
	}
}
    ...
    m_generic_goals         ()
{
	...
	Agent_Owning_List::const_iterator agent_ptr_iter = scheduler.m_agents.begin();
	while(agent_ptr_iter != scheduler.m_agents.end())
	{
		m_agents.push_back(std::make_unique<Agent>(**agent_ptr_iter));
		++agent_ptr_iter;
	}
	...
	for(size_t    goal_type = 0;
		...
		m_generic_goals.push_back(std::make_unique<Goal>(**generic_goals_iter));
		...
		{
			m_goals_of_type[goal_type].emplace_back(Goal::BAD_UTILITY, std::make_unique<Goal>(*sorted_goal_iter->second));
			++sorted_goal_iter;
		}
	}
}
...
void Scheduler::Cleanup()
{
	...
	for
	(
	    Agent_Owning_List::iterator agent_ptr_iter  = m_agents.begin();
	                                agent_ptr_iter != m_agents.end();
	                              ++agent_ptr_iter
	)
	{
		Agent_Owning_List::iterator agent2_ptr_iter = agent_ptr_iter;
		++agent2_ptr_iter;

		while(agent2_ptr_iter != m_agents.end())
		{
			Assert(agent_ptr_iter->get() != agent2_ptr_iter->get());
...
void Scheduler::Process_Agent_Changes()
{
	...
	{
		Agent* theAgent = agent_ptr_iter->get();
	...
	// GetTickCount() returns uint32 but t1 is time_t; narrow the ms diff to match %d.
...
void Scheduler::Sort_Goals()
{
	...
	AI_DPRINTF(k_DBG_SCHEDULER_ALL, m_playerId, -1, -1,
		("//  %zu goals sorted = %d ms, Player %d\n\n\n", m_goals.size(), static_cast<sint32>(t2 - t1), m_playerId));
	...
	for
	(
	    Agent_Owning_List::iterator agent_iter  = m_agents.begin();
...
void Scheduler::Match_Resources(const bool move_armies)
{
	...
	)
	{
		...
		AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_ptr->Get_Goal_Type(), -1,
			...
			("[%d] Starting to match resources to %s: %lx (x=%d,y=%d), match %d, %s\n",
				count, g_theGoalDB->Get(goal_ptr->Get_Goal_Type())->GetNameText(), reinterpret_cast<uintptr_t>(goal_ptr), pos.x, pos.y, oldMatchValue, (world_Get()->HasCity(pos) ? world_Get()->GetCity(pos).GetName() : "field")));
		count++;
#endif
		...
		{
			AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_ptr->Get_Goal_Type(), -1,
				("\t\tGOAL (goal: %lx) -- First goal with bad utility - stop matching\n",
					reinterpret_cast<uintptr_t>(goal_ptr)));
		...
		{
			AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_ptr->Get_Goal_Type(), -1,
			("\t\tGOAL (goal: %lx)(agent count: %d) -- Goal with bad utility, trying agian in next cycle.\n",
					reinterpret_cast<uintptr_t>(goal_ptr), goal_ptr->Get_Agent_Count()));
	...
	//		Assert(goal_ptr->Get_Agent_Count() == 0);
		...
		if(goal_ptr->Needs_Transporter())
		{
			...
			|| transNum == goal_ptr->Get_Transporters_Num()
				...
				AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_ptr->Get_Goal_Type(), -1,
				("\t\tGOAL_FAILED Not enough transporters (goal: %lx)\n", reinterpret_cast<uintptr_t>(goal_ptr)));
		...
		{
			AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_ptr->Get_Goal_Type(), -1,
				("\t\tGOAL (goal: %lx) -- No agents were committed, maybe next time. Continuing...\n",
					reinterpret_cast<uintptr_t>(goal_ptr)));
			continue;
		}
		...
		switch (result)
		{
			...
			case GOAL_COMPLETE:
			{
			// The second conversion lost its argument in the Squad->Agent merge
			// (929d60cb): a completed goal has N agents, no single one to print.
			AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_ptr->Get_Goal_Type(), -1,
				("\t\tGOAL_COMPLETE (goal: %lx)\n", reinterpret_cast<uintptr_t>(goal_ptr)));

				committed_agents -= goal_ptr->Get_Agent_Count();
				if(goal_ptr->Get_Removal_Time())
				{
					AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_ptr->Get_Goal_Type(), -1,
					("\t\tGOAL_COMPLETE (goal: %lx) -- Removing matches for goal.\n",
					reinterpret_cast<uintptr_t>(goal_ptr)));
...
//
///////////////////////////////////////////////////////////////////////////////////////////////////////
void Scheduler::Add_New_Goal(std::unique_ptr<Goal> new_goal)
{
	...
	{
		Goal_ptr old_goal = tmp_goal_iter->second.get();
	...
	m_goals_of_type[goal_type].
		emplace_back(Goal::BAD_UTILITY, std::move(new_goal));
}
...
//
/////////////////////////////////////////////////////////////////////////////////////////////////
void Scheduler::Add_New_Agent(std::unique_ptr<Agent> new_agent)
{
	...
	else
	{
		...
		Agent_ptr agent_ptr = new_agent.get();
		Add_Agent(std::move(new_agent));

		Add_New_Matches_For_Agent(agent_ptr);
	}
}
...
//
/////////////////////////////////////////////////////////////////////////////////////////////////
Agent_Owning_List::iterator Scheduler::Add_Agent(std::unique_ptr<Agent> agent)
{
#ifdef _DEBUG_SCHEDULER

	Agent_Owning_List::iterator agent_iter = m_agents.begin();
	while (agent_iter != m_agents.end() && !(*agent_iter)->ContainsArmyIn(agent.get()))
	{
		agent_iter++;
	}
	...
	return m_agents.insert(m_agents.end(), std::move(agent));
}
...
Scheduler::Sorted_Goal_Iter Scheduler::Remove_Goal(const Scheduler::Sorted_Goal_Iter & sorted_goal_iter)
{
	...
	Remove_Matches_For_Goal(sorted_goal_iter->second.get());
...
sint32 Scheduler::GetValueUnsatisfiedGoals(const GOAL_TYPE & type) const
{
	...
	if(IsValid(type, m_goals_of_type))
	{
		...
		{
			Goal_ptr const goal_ptr = sorted_goal_iter.second.get();
...
bool Scheduler::Prioritize_Goals()
{
	...
	for(goal_type = 0; goal_type < g_theGoalDB->NumRecords(); goal_type++)
	{
		...
		AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_type, -1,("\n"));
		AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_type, -1,("//\n"));
		AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_type, -1,("// Calculate raw priority for %s (%zu)\n", g_theGoalDB->Get(goal_type)->GetNameText(), m_goals_of_type[goal_type].size()));
		AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_type, -1,("//\n"));
		AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_type, -1,("\n"));
		AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_type, -1,
			("\t %9lx,\tGOAL,\t\tRAW_PRIORITY,\t\tCOORDS,\t\tINIT_VALUE,\tLAST_VALUE,\tTHREAT,\t\tENEMYVAL,\tALLIEDVAL,\tMAXPOW,\t\tHOMEDIST\t(   ),\t\tENEMYDIST (    ),\t\tSETTLE,\t\tCHOKE,\t\tUNEXPLORED,\tNOT_VISIBLE,\tIN_HOME_TER,\tIN_ENEMY_TER,\tON_NEUT_TER,\tSLAVERY_PROTECTION,\tSMALL_CITY,\tTHREATEN,\tCONNECTED,\tSMALL_EMP,\tWEAKEST\n",
		reinterpret_cast<uintptr_t>(this)));

		      sorted_goal_iter  = m_goals_of_type[goal_type].begin();
		while(sorted_goal_iter != m_goals_of_type[goal_type].end())
		{
			Goal_ptr goal_ptr = sorted_goal_iter->second.get();

			if(goal_ptr->Get_Removal_Time())
			{
				AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_ptr->Get_Goal_Type(), -1,
				("\tRemoving Invalid/Completed Goal: %lx\n", reinterpret_cast<uintptr_t>(goal_ptr)));
	...
	for(goal_type = 0; goal_type < g_theGoalDB->NumRecords(); goal_type++)
	{
		...
		AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_type, -1,("\n"));
		AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_type, -1,("//\n"));
		AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_type, -1,("// Calculate priority for %s (%zu)\n", g_theGoalDB->Get(goal_type)->GetNameText(), m_goals_of_type[goal_type].size()));
		AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_type, -1,("//\n"));
		AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_type, -1,("\n"));
//		AI_DPRINTF(k_DBG_SCHEDULER, m_playerId, goal_type, -1,
...
//		this));
		...
		Remove_Matches_For_Goal(generic_goal_iter->get());
		Add_New_Matches_For_Goal(generic_goal_iter->get(), false);
		...
		)
		{
			...
			if(sorted_goal_iter->first > Goal::BAD_UTILITY)
			{
				...
				{
					sorted_goal_iter->first = goal_ptr->Compute_Matching_With_Generic_Matches(generic_goal_iter->get());
				}
				else
				{
		...
		AI_DPRINTF(k_DBG_SCHEDULER_ALL, m_playerId, goal_type, -1, ("\t//\n"));
		AI_DPRINTF(k_DBG_SCHEDULER_ALL, m_playerId, goal_type, -1, ("\t// SORTED GOALS of %s (%zu)\n", g_theGoalDB->Get(goal_type)->GetNameText(), m_goals_of_type[goal_type].size()));
		AI_DPRINTF(k_DBG_SCHEDULER_ALL, m_playerId, goal_type, -1, ("\t//\n"));
		AI_DPRINTF(k_DBG_SCHEDULER_ALL, m_playerId, goal_type, -1, ("\n"));
	}
...
bool Scheduler::Prune_Goals()
{
	...
	for(sint32 i = 0; i < strategy.GetNumGoalElement(); i++)
	{
		...
		if (goal_type < 0 || static_cast<size_t>(goal_type) >= m_goals_of_type.size())
			continue;
...
//		max_eval = m_goals_of_type[goal_type].size(); // Temporary, maybe permanetly
		...
		while(goal_ptr_iter != pruned_goal_iter &&
			...
			Goal_ptr goal_ptr     = goal_ptr_iter->second.get();
			...
			if(ok_to_match_goal)
			{
				if(goal_ptr->Get_Matches_Num() == 0)
				{
					goal_ptr->Copy_Insert_Matches(m_generic_goals[goal_type].get());
				}
...
#if defined(_DEBUG) || defined(USE_LOGGING)
				AI_DPRINTF(k_DBG_SCHEDULER_ALL, m_playerId, goal_type, -1,
				("\t%3d: [%lx]", count, reinterpret_cast<uintptr_t>(goal_ptr_iter->second.get())));
...
)
{
	...
	for
	(auto const & agent : m_agents)
	{
			if((goal_squad_class & agent->Get_Squad_Class()) != goal_squad_class)
			continue;

		goal_ptr->Add_Match(agent.get(), update_match_value);
	}
}
...
)
{
	...
	for (auto const & goal : m_generic_goals)
...
)
{
	...
	AI_DPRINTF(k_DBG_SCHEDULER,  m_playerId, -1, -1, ("\t%d agents to roll back for Goal %lx, %s.\n", goal->Get_Agent_Count(), reinterpret_cast<uintptr_t>(goal), g_theGoalDB->Get(goal->Get_Goal_Type())->GetNameText()));
...
)
{
	...
	(auto const & agent : m_agents)
	{
		...
		if(goal_ptr->Cannot_Be_Used(agent.get())) // Failed transporters
		{
			continue;
		}

		bool hasMatch  = goal_ptr->Has_Agent_And_Set_Needs_Cargo(agent.get());
		...
		&& goal_ptr->CanReachTargetContinent(agent.get())
		&& goal_ptr->Add_Transport_Match(agent.get())
		){
			match_added = true;
		}
	}
...
void Scheduler::SetArmyDetachState(const Army & army, const bool detach)
{
	...
	{
		Agent_ptr ctp_agent = agent_ptr_iter->get();
...
{
	Agent_Owning_List::const_iterator agent_ptr_iter = m_agents.begin();

	while(agent_ptr_iter != m_agents.end())
	{
		const Agent_ptr ctp_agent = agent_ptr_iter->get();
		if (ctp_agent->Get_Army().m_id == army.m_id)
		{
...
//			continue;
		  ...
		  )
		{
			...
			{
				garrisonAgents[idx].emplace_back(defense_strength, agent.get());
			}
		}
	}
	...
	for(sint32 i = 0; i < cityNum; ++i)
	{
		...
		)
		{
			...
			{
			// Army is an ID handle: print the ID (uint32 m_id), not the wrapper object.
			AI_DPRINTF(k_DBG_SCHEDULER_DETAIL, m_playerId, -1, -1,("%9lx\t %9x\t %s\n", reinterpret_cast<uintptr_t>(agent_iter->second), agent_iter->second->Get_Army().m_id, city.GetName()));
			...
			AI_DPRINTF(k_DBG_SCHEDULER_DETAIL, m_playerId, -1, -1,("%9lx\t %9x\t %s (Not needed for city garrison)\n", reinterpret_cast<uintptr_t>(agent_iter->second), agent_iter->second->Get_Army().m_id, city.GetName()));
			}
		}
	}
