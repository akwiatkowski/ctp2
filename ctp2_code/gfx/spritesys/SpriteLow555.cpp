...
void Sprite::DrawLowClipped555(Pixel16 *frame, sint32 drawX, sint32 drawY, sint32 width, sint32 height,
								...
								if (flags & k_BIT_DRAWFLAGS_FOGGED) {
									...
									if (flags & k_BIT_DRAWFLAGS_DESATURATED) {
										while (len) {
											len--;
											if (xpos >= xstart && xpos < xend)
												*destPixel++ = pixelutils_Desaturate_555(*rowData++);
											else
												rowData++;
											xpos++;
										}
				...
				switch ((tag & 0x0F00) >> 8)
				{
					...
					case k_COPY_RUN_ID			:
						 ...
						 if(flags&k_BIT_DRAWFLAGS_SPECIAL1)
						 {
							...
							if (flags & k_BIT_DRAWFLAGS_TRANSPARENCY)
							{
								__BlendFast_555_16(destAddr,rowAddr,1,1,w_len,transparency);
								break;
							}

							if (flags & k_BIT_DRAWFLAGS_FOGGED)
							{
								__Shadow_555_32(destAddr,rowAddr,1,1,w_len>>1);
								__Shadow_555_16(destAddr,rowAddr,1,1,w_len&0x01);
								break;
							}
					...
					case k_SHADOW_RUN_ID		:
						 ...
						 __Shadow_555_32(destAddr,1,w_len>>1);
						 __Shadow_555_16(destAddr,1,w_len&0x01);
...
void Sprite::DrawLowReversed555(Pixel16 *frame, sint32 drawX, sint32 drawY, sint32 width, sint32 height,
							 uint16 transparency, Pixel16 outlineColor, uint16 flags)
{
	uint8			*surfBase;
					...
					if (flags & k_BIT_DRAWFLAGS_AA) {
						if (flags & k_BIT_DRAWFLAGS_AA_4_WAY) {
							if (pixel1 != k_MEDIUM_KEY || pixel2 != k_MEDIUM_KEY || pixel3 != k_MEDIUM_KEY || pixel4 != k_MEDIUM_KEY) {
								pixel =	average_555(pixel1, pixel2, pixel3, pixel4);
							...
							if (pixel1 != k_MEDIUM_KEY || pixel2 != k_MEDIUM_KEY) {
								pixel = average_555(pixel2, pixel3);
								if (pixel == 0)
									pixel = 0x0001;
...
void Sprite::DrawFlashLow555(Pixel16 *frame, sint32 drawX, sint32 drawY, sint32 width, sint32 height,
					 uint16 transparency, Pixel16 outlineColor, uint16 flags)
{
	uint8			*surfBase;
				...
				switch ((tag & 0x0F00) >> 8) {
					...
					case k_COPY_RUN_ID			: {
							...
							while (len--) {
								...
								*destPixel = pixelutils_Additive_555(*destPixel, *rowData);
								destPixel++;
								rowData++;
							}
						}
...
void Sprite::DrawFlashLowReversed555(Pixel16 *frame, sint32 drawX, sint32 drawY, sint32 width, sint32 height,
							 uint16 transparency, Pixel16 outlineColor, uint16 flags)
{
	uint8		*surfBase;
				...
				switch ((tag & 0x0F00) >> 8) {
					...
					case k_COPY_RUN_ID			: {
							...
							if (flags & k_BIT_DRAWFLAGS_TRANSPARENCY) {
								...
								for (uint16 i=0; i<len; i++) {
									*destPixel = pixelutils_Additive_555(*rowData, *destPixel);
									destPixel--;
									rowData++;
								}
							}
...
void Sprite::DrawFlashScaledLow555(Pixel16 *data, sint32 x, sint32 y, sint32 destWidth, sint32 destHeight,
							 uint16 transparency, Pixel16 outlineColor, uint16 flags, BOOL reverse)
{
	uint8			*surfBase;
...
inline Pixel16 Sprite::average_555(Pixel16 pixel1, Pixel16 pixel2)
{
	uint16		 r1;
	uint16		 g1;
	uint16		 b1;
...
inline void Sprite::__Desaturate_555_16	(PixelAddress &dest,PixelAddress &src,sint32 dest_inc,sint32 src_inc,sint32 num)
{
	while (num)
	{
		num--;
		*dest.w_ptr=pixelutils_Desaturate_555(*src.w_ptr);
		dest.w_ptr +=dest_inc;
		src.w_ptr  +=src_inc;
	}
}
