...
// P11 2b (ADR-001) — render the GPU world layer at the oversized (margin) view.
//
// Draws terrain (RepaintTiles) + unit/city actors (RepaintSprites) directly into
// the oversized world surface, with the map view TEMPORARILY widened by a whole-
// tile margin so the border holds real content (terrain AND actors) for the
// sub-tile GPU pan to slide into without a black edge. The widen is save/restored
// within this one call, so gameplay + mouse coordinates (which use the screen
// view) are untouched. Publishes the whole-tile pixel offset at which the screen
// top-left sits inside the surface, so the present + oracle window to the exact
// centre. Replaces the old screen-mirror of the world layer (aui_UI::BltToSecondary).
// Called from the background draw handler; no-op unless GPU layers are on.
void TiledMap::RenderWorldLayer(aui_Surface *worldSurf)
{
	if (!worldSurf || !m_tileSet || !m_localVision) return;

	sint32 const tileW   = GetZoomTilePixelWidth();
	sint32 const vscroll = GetZoomTilePixelHeight() / 2;
	if (tileW < 1 || vscroll < 1) return;

	// Whole-tile margin that fits inside the surface's allocation margin, so the
	// projection shift is an exact integer number of tiles (no sub-tile skew).
	sint32 const allocMargin = aui_SDL::WorldMargin();
	sint32 const mtX = allocMargin / tileW;      // whole tile columns
	sint32 const mtY = allocMargin / vscroll;    // whole half-rows
	if (mtX < 1 || mtY < 1) return;
	aui_SDL::SetWorldContentOffset(mtX * tileW, mtY * vscroll);

	// --- save the state we clobber ---
	aui_Surface * const savedSurface  = m_surface;
	RECT const          savedView     = m_mapViewRect;
	RECT const          savedSurfRect = m_surfaceRect;

	// Widen the view by the margin: shrinking left/top shifts the projection
	// origin back so the screen's top-left lands at (mtX*tileW, mtY*vscroll)
	// inside the surface, and growing right/bottom includes the margin tiles.
	m_mapViewRect.left   -= mtX;
	m_mapViewRect.right  += mtX;
	m_mapViewRect.top    -= mtY;
	m_mapViewRect.bottom += mtY;
	m_surfaceRect.left = 0; m_surfaceRect.top = 0;
	m_surfaceRect.right = worldSurf->Width(); m_surfaceRect.bottom = worldSurf->Height();

	RetargetTileSurface(worldSurf);

	RECT widerRect = m_mapViewRect;

	// Terrain: raw-pixel writes, so lock the surface (mirrors RenderFullMap).
	LockThisSurface(worldSurf);
	if (m_surfBase) memset(m_surfBase, 0, (size_t) m_surfHeight * m_surfPitch);
	RepaintTiles(&widerRect);
	UnlockSurface();

	// Actors (units/cities): the blitter self-locks (mirrors background_draw_handler).
	RepaintSprites(worldSurf, &widerRect, false);

	// --- restore: gameplay/mouse keep the screen view ---
	RetargetTileSurface(savedSurface);
	m_mapViewRect = savedView;
	m_surfaceRect = savedSurfRect;
}
