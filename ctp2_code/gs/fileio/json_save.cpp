...
void from_json(nlohmann::json const &j, Cell &c)
{
    ...
    if (j.contains("objects"))
    {
        for (auto const &id : j.at("objects"))
        {
            if (!c.m_objects)
            {
                c.m_objects = std::make_unique<DynamicArray<ID>>();
            }
...
void from_json(nlohmann::json const &j, World &w)
{
    ...
    {
        w.m_goodValue = std::make_unique<double[]>(n_goods);
...
void from_json(nlohmann::json const &j, Happy &h)
{
    ...
    {
        h.m_tracker = std::make_unique<HappyTracker>();
...
void to_json(nlohmann::json &j, BuildQueue const &q)
{
    ...
    PointerList<BuildNode>::Walker walk(q.m_list.get());
    while (walk.IsValid())
    {
...
void from_json(nlohmann::json const &j, BuildQueue &q)
{
    ...
    {
        BuildNode *node = std::make_unique<BuildNode>().release();
...
void from_json(nlohmann::json const &j, FeatTracker &ft)
{
    ...
    {
        Feat *feat = std::make_unique<Feat>(0, 0).release();  // dummy ctor args; overwritten by JSON
...
// + m_point + m_path. m_gameEventArgs (unique_ptr<GameEventArgList>) remains
...
void from_json(nlohmann::json const &j, Order &o)
{
    ...
    if (j.contains("path") && !j.at("path").is_null()) {
        o.m_path = std::make_unique<Path>();
        j.at("path").get_to(*o.m_path);
    } else {
        o.m_path.reset();
    }
    // m_gameEventArgs stays null — transient, not persisted (unique_ptr)
}
...
void to_json(nlohmann::json &j, ArmyData const &a)
{
    ...
    if (a.m_orders)
    {
        PointerList<Order>::Walker walk(a.m_orders.get());
        while (walk.IsValid())
        {
...
void from_json(nlohmann::json const &j, ArmyData &a)
{
    ...
    if (!a.m_attackedByDefenders) a.m_attackedByDefenders = std::make_unique<UnitDynamicArray>();
    ...
    if (!a.m_orders) a.m_orders = std::make_unique<PointerList<Order>>();
    a.m_orders->DeleteAll();
    for (auto const &order_json : j.at("orders"))
    {
        Order *order = std::make_unique<Order>().release();
...
void from_json(nlohmann::json const &j, UnitData &u)
{
    ...
    {
        u.m_cargo_list = std::make_unique<UnitDynamicArray>();
    ...
    if (!j.at("city_data").is_null())
    {
        ...
        u.m_city_data = std::make_unique<CityData>(0, Unit(0), MapPoint(0, 0));
    ...
    {
        u.m_roundTheWorldMask = std::make_unique<BitMask>(1);  // dummy size; replaced by from_json
...
void from_json(nlohmann::json const &j, InstallationPool &p)
{
    ...
    {
        InstallationData *data = std::make_unique<InstallationData>(ID(0)).release();
        entry.get_to(*data);
        p.Insert(data);
    }
}
...
void from_json(nlohmann::json const &j, TradeRouteData &d)
{
    ...
    if (!d.m_astarPath) d.m_astarPath = std::make_unique<Path>().release();
...
void from_json(nlohmann::json const &j, TradePool &p)
{
    ...
    {
        TradeRouteData *data = std::make_unique<TradeRouteData>(TradeRoute(0)).release();
        entry.get_to(*data);
        p.Insert(data);
...
void from_json(nlohmann::json const &j, TerrainImprovementPool &p)
{
    ...
    {
        TerrainImprovementData *data = std::make_unique<TerrainImprovementData>(ID(0)).release();
        entry.get_to(*data);
        p.Insert(data);
    }
}
...
void from_json(nlohmann::json const &j, DiplomaticRequestPool &p)
{
    ...
    {
        DiplomaticRequestData *data = std::make_unique<DiplomaticRequestData>(ID(0), /*currentRound*/0).release();
        entry.get_to(*data);
        p.Insert(data);
    }
}
...
void from_json(nlohmann::json const &j, AgreementPool &p)
{
    ...
    {
        AgreementData *data = std::make_unique<AgreementData>(ID(0)).release();
        entry.get_to(*data);
        p.Insert(data);
    }
}
...
void from_json(nlohmann::json const &j, TradeOfferPool &p)
{
    ...
    {
        TradeOfferData *data = std::make_unique<TradeOfferData>(ID(0)).release();
        entry.get_to(*data);
        p.Insert(data);
    }
}
...
{
    std::unique_ptr<MBCHAR[]> old(dest);
    if (j.is_null())
    {
        dest = nullptr;
        return;
    ...
    dest = std::make_unique<MBCHAR[]>(s.size() + 1).release();
...
void from_json(nlohmann::json const &j, CivilisationPool &p)
{
    ...
    {
        CivilisationData *data = std::make_unique<CivilisationData>(ID(0)).release();
        entry.get_to(*data);
        p.Insert(data);
    }
...
void from_json(nlohmann::json const &j, UnitPool &p)
{
    ...
    {
        UnitData *data = std::make_unique<UnitData>(entry).release();
        p.Insert(data);
...
void from_json(nlohmann::json const &j, ArmyPool &p)
{
    ...
    {
        ArmyData *data = std::make_unique<ArmyData>(Army(0)).release();
        entry.get_to(*data);
        p.Insert(data);
    }
}
...
void from_json(nlohmann::json const &j, CityData &c)
{
    ...
    {
        if (!c.m_happy) c.m_happy = std::make_unique<Happy>();
...
void to_json(nlohmann::json &j, Player const &p)
{
    ...
    j = nlohmann::json{
        ...
        {"all_armies",       ids_from_armies(p.m_all_armies.get())},
        {"all_cities",       ids_from_units(p.m_all_cities.get())},
        {"all_units",        ids_from_units(p.m_all_units.get())},
...
void from_json(nlohmann::json const &j, Player &p)
{
    ...
    load_army_ids("all_armies",   p.m_all_armies.get());
    load_unit_ids("all_cities",   p.m_all_cities.get());
    load_unit_ids("all_units",    p.m_all_units.get());
...
void from_json(nlohmann::json const &j, SlicSymbolData &s)
{
    ...
    switch (type)
    {
        ...
        case SLIC_SYM_ARRAY:
            ...
            s.m_val.m_array = std::make_unique<SlicArray>(SS_TYPE_BAD, SLIC_SYM_UNDEFINED).release();
        ...
        case SLIC_SYM_STRUCT:
        {
            ...
            s.m_val.m_struct = std::make_unique<SlicStructInstance>(desc).release();
...
// nullptr when j is JSON null.  Caller owns the result (release()).
std::unique_ptr<SlicSymbolData> loadSlicSymbolFromJson(nlohmann::json const &j)
{
    if (j.is_null())
        return nullptr;
    ...
    if (serial == "named")
    {
        auto p = std::make_unique<SlicNamedSymbol>();
        from_json(j, *p);
        return p;
    }
    if (serial == "parameter")
    {
        auto p = std::make_unique<SlicParameterSymbol>();
        from_json(j, *p);
        return p;
    }
    if (serial == "builtin")
    {
        auto p = std::make_unique<SlicBuiltinNamedSymbol>();
        from_json(j, *p);
        return p;
    }
    // "generic" or missing — base SlicSymbolData.
    auto p = std::make_unique<SlicSymbolData>();
    from_json(j, *p);
    return p;
}
...
void from_json(nlohmann::json const &j, SlicArray &a)
{
    ...
    if (a.m_type == SS_TYPE_SYM)
    {
        for (size_t i = 0; i < a.m_allocatedSize; ++i)
            std::unique_ptr<SlicSymbolData>(a.m_array[i].m_sym);
    }
    a.m_array.reset();
    ...
    {
        for (sint32 i = 0; i < a.m_arraySize; ++i)
            a.m_array[i].m_sym = loadSlicSymbolFromJson(elements[i]).release();
    }
}
...
void to_json(nlohmann::json &j, SlicStructInstance const &s)
{
    ...
    for (sint32 i = 0; i < numMembers; ++i)
        members.push_back(storeSlicSymbolToJson(s.m_members[i].get()));
...
void from_json(nlohmann::json const &j, SlicStructInstance &s)
{
    ...
    {
        s.m_ownedData = loadSlicSymbolFromJson(j.at("data_symbol"));
        s.m_dataSymbol = s.m_ownedData.get();
    }
    else if (!s.m_dataSymbol && s.m_dataSymbolIndex < 0)
    {
        ...
        s.m_ownedData   = s.m_description->CreateDataSymbol();
        s.m_dataSymbol  = s.m_ownedData.get();
...
// as []. std::vector lists serialise as plain arrays.
...
template <class T>
std::unique_ptr<SimpleDynamicArray<T>> jsonToSda(nlohmann::json const &j)
{
    if (j.is_null())
        return nullptr;
    auto out = std::make_unique<SimpleDynamicArray<T>>();
    for (auto const &el : j)
    {
        T value;
        el.get_to(value);
...
{
    j = nlohmann::json::object();
    j["cities"]       = sdaToJson(c.m_cityList.get());
    j["units"]        = sdaToJson(c.m_unitList.get());
    j["armies"]       = sdaToJson(c.m_armyList.get());
    j["players"]      = sdaToJson(c.m_playerList.get());
    j["ints"]         = sdaToJson(c.m_intList.get());
    j["unit_records"] = sdaToJson(c.m_unitRecordList.get());
    j["locations"]    = sdaToJson(c.m_locationList.get());
    j["agreements"]   = sdaToJson(c.m_agreementList.get());
    j["trade_offers"] = sdaToJson(c.m_tradeOffersList.get());
    j["goods"]        = sdaToJson(c.m_goodList.get());
    j["governments"]  = sdaToJson(c.m_governmentList.get());
    j["advances"]     = sdaToJson(c.m_advanceList.get());

    j["calamities"]     = c.m_calamityList;
    j["golds"]          = c.m_goldList;
    j["ranks"]          = c.m_rankList;
    j["wonders"]        = c.m_wonderList;
    j["orders"]         = c.m_orderList;
    j["madlib_choices"] = c.m_madlibChoiceList;
    j["madlib_names"]   = c.m_madlibNameList;
    j["attitudes"]      = c.m_attitudeList;
    j["ages"]           = c.m_ageList;
    j["buildings"]      = c.m_buildingList;
    j["trade_bids"]     = c.m_tradeBidList;
...
{
    // unique_ptr/vector members free their own storage on assignment.
    ...
    j.at("calamities")    .get_to(c.m_calamityList);
    j.at("golds")         .get_to(c.m_goldList);
    j.at("ranks")         .get_to(c.m_rankList);
    j.at("wonders")       .get_to(c.m_wonderList);
    j.at("orders")        .get_to(c.m_orderList);
    j.at("madlib_choices").get_to(c.m_madlibChoiceList);
    j.at("madlib_names")  .get_to(c.m_madlibNameList);
    j.at("attitudes")     .get_to(c.m_attitudeList);
    j.at("ages")          .get_to(c.m_ageList);
    j.at("buildings")     .get_to(c.m_buildingList);
    j.at("trade_bids")    .get_to(c.m_tradeBidList);
...
void from_json(nlohmann::json const &j, SlicObject &o)
{
    ...
    o.m_recipientList = o.m_numRecipients > 0
                           ? std::make_unique<sint32[]>(o.m_numRecipients)
                           : nullptr;
    for (sint32 i = 0; i < o.m_numRecipients; ++i)
    ...
    o.m_frame = o.m_segment ? std::make_unique<SlicFrame>(o.m_segment) : nullptr;
...
void to_json(nlohmann::json &j, SlicSegment const &s)
{
    ...
    for (sint32 i = 0; i < s.m_num_parameters; ++i)
    {
        ...
        if (!s.m_parameter_symbols.empty())
            paramIdx.push_back(
...
void from_json(nlohmann::json const &j, SlicSegment &s)
{
    ...
    s.m_trigger_symbols.clear();
    s.m_parameter_symbols.clear();
...
void from_json(nlohmann::json const &j, SlicSymTab &t)
{
    ...
    for (sint32 i = 0; i < t.m_numEntries && i < static_cast<sint32>(entries.size()); ++i)
    {
        ...
        // symtab entry).
        auto *named = dynamic_cast<SlicNamedSymbol *>(sym.get());
        if (!named)
        {
            throw nlohmann::json::other_error::create(
                501,
...
void to_json(nlohmann::json &j, SlicEngine const &e)
{
    ...
    j["segments"]  = stringHashToJson<SlicSegment>(e.m_segmentHash.get());
    j["constants"] = stringHashToJson<SlicConst>(e.m_constHash.get());
    ...
    j["current_message"]  = e.m_currentMessage
        ...
        : 0u;
    j["disabled_classes"] = sdaToJson(e.m_disabledClasses.get());
}
...
void from_json(nlohmann::json const &j, SlicEngine &e)
{
    ...
    if (e.m_segmentHash)
    {
        ...
        {
            auto *seg = std::make_unique<SlicSegment>().release();
            it->get_to(*seg);
    ...
    if (e.m_constHash)
    {
        ...
        {
            auto *c = std::make_unique<SlicConst>("", 0).release();
            cj.get_to(*c);
    ...
    for (auto & m_record : e.m_records)
    {
        ...
        {
            m_record->DeleteAll();
            std::unique_ptr<PointerList<SlicRecord>>{m_record};
            m_record = nullptr;
        }
    }
    for (auto const &rec : j.at("records"))
    {
        ...
        e.m_records[p] = std::make_unique<PointerList<SlicRecord>>().release();
        for (auto const &entryJson : rec.at("entries"))
        {
            auto *r = std::make_unique<SlicRecord>(0, nullptr, nullptr, nullptr).release();
            entryJson.get_to(*r);
    ...
    if (!e.m_disabledClasses)
        e.m_disabledClasses = std::make_unique<SimpleDynamicArray<sint32>>();
}
...
void from_json(nlohmann::json const &j, SlicButton &b)
{
    ...
    if (!b.m_message)
        b.m_message = std::make_unique<Message>();
    j.at("message").get_to(*b.m_message);

    auto newContext = std::make_unique<SlicObject>();
    j.at("context").get_to(*newContext);
    newContext->AddRef();
    if (b.m_context)
        b.m_context->Release();
    b.m_context = newContext.release();
...
void from_json(nlohmann::json const &j, SlicEyePoint &e)
{
    ...
    if (!e.m_message)
        e.m_message = std::make_unique<Message>();
...
void to_json(nlohmann::json &j, MessageData const &m)
{
    ...
    if (m.m_buttonList)
    {
        PointerList<SlicButton>::Walker walk(m.m_buttonList.get());
        while (walk.IsValid())
        {
    ...
    if (m.m_eyePoints)
    {
        PointerList<SlicEyePoint>::Walker walk(m.m_eyePoints.get());
        while (walk.IsValid())
        {
...
void from_json(nlohmann::json const &j, MessageData &m)
{
    ...
    if (m.m_buttonList)
    {
        ...
        m.m_buttonList.reset();
    }
    m.m_buttonList = std::make_unique<PointerList<SlicButton>>();
    for (auto const &bj : j.at("buttons"))
    {
        auto *btn = std::make_unique<SlicButton>().release();
        bj.get_to(*btn);
    ...
    m.m_eyePoints = std::make_unique<PointerList<SlicEyePoint>>();
    for (auto const &ej : j.at("eye_points"))
    {
        auto *eye = std::make_unique<SlicEyePoint>().release();
        ej.get_to(*eye);
        m.m_eyePoints->AddTail(eye);
    }

    if (!m.m_cityList)
        m.m_cityList = std::make_unique<UnitDynamicArray>();
    m.m_cityList->Clear();
...
void from_json(nlohmann::json const &j, MessagePool &p)
{
    ...
    {
        auto *data = std::make_unique<MessageData>(ID(0), 0).release();
        entry.get_to(*data);
        p.Insert(data);
    }
}
        ...
        if (doc.contains("players") && player_arr_Get())
        {
            ...
            for (sint32 i = 0; i < n; ++i)
            {
                ...
                if (!alive)
                {
                    ...
                    std::unique_ptr<Player>{g_players[i]};
                    g_players[i] = nullptr;
                }
