...
//   properties. - April 24th 2005 Martin G�hmann
...
//   Actual this should be replaced by new order. - April 30th 2005 Martin G�hmann
// - Implemented GovernmentModified for UnitDB - April 30th 2005 Martin G�hmann
...
// - Positions of units on transports are now updated. (Sep 9th 2005 Martin G�hmann)
...
// - Armies with a CanSettleOn only unit are now recognized as settler armies. (May 21st 2006 Martin G�hmann)
...
// - Fixed CanCloak (25th 2006 Martin G�hmann)
...
// - Units that are grouped into previously empty armies now show up on the map. (5-Aug-2007 Martin G�hmann)
// - Improved Ungroup and transport capacity methods. (5-Aug-2007 Martin G�hmann)
// - Bombard order is not given twice anymore. (30-Jan-2008 Martin G�hmann)
...
// - Throwing diplomatic parties costs now the displayed price. (9-Feb-2008 Martin G�hmann)
// - Added messages for failed slave raids, one for an alive slave and another
//   for a killed slaver. (9-Feb-2008 Martin G�hmann)
// - Expelling costs now move points. (9-Feb-2008 Martin G�hmann)
...
void ArmyData::RemainNumUnits(sint32 remain)
{
	...
	{
		Army newArmy = player_Get(m_owner)->GetNewArmy(CAUSE_NEW_ARMY_UNGROUPING_ORDER);
		gevmanager_Get()->AddEvent(GEV_INSERT_AfterCurrent, GEV_AddUnitToArmy,
		                       GEA_Unit, m_array[i],
		                       GEA_Army, newArmy,
                                  ...
                                  bool isCargoPodCheck)
{
    ...
    for (sint32 i = 0; i < n; i++) {
        ...
        if (!player_Get(owner)->HasWarWith(input[i].GetOwner()))
            continue;
...
bool ArmyData::CheckActiveDefenders(MapPoint &pos, bool cargoPodCheck)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (!ta.CanSee(Army(m_id)))
            continue;
        Unit        td      =
            GetTopVisibleUnit(player_view::VisiblePlayer());
		...
		Cell *dcell = world_Get()->GetCell(dpos);
...
// Parameters : -
//
// Globals    : world_Get:             World properties
//              player array:               List of players
...
//		if(world_Get()->GetArmy(it.Pos()), owner != m_owner
...
//END EMOD
        ...
        if(cell) {
            ...
            for(i = 0; i < cell->GetNumTradeRoutes(); i++) {
                ...
                if(route->GetPiratingArmy().m_id == m_id) {
                    player_Get(m_owner)->AddGold(static_cast<sint32>(route->GetValue() * g_theConstDB->Get(0)->GetPiracyWasteCoefficient()));
    ...
    if (!player_Get(m_owner)->m_can_use_sea_tab)
    {
        player_Get(m_owner)->m_can_use_sea_tab = (CountMovementTypeSea() > 0);
    }
...
//
// Globals    : world_Get             : The Game's world
//
...
// Parameters : MapPoint
//
// Globals    : player array                : player array
//            : g_network               :
//            : g_theAdvanceDB
//            : slicengine_Get()
//
...
ORDER_RESULT ArmyData::Franchise(const MapPoint &point)
{
	...
	Unit city   = world_Get()->GetCity(point);
		...
		for(i = 0; i < n; i++) {
			if(foundCity) {
				...
				if(expelled[i].GetArmy().Num() > 1) {
					newArmy = player_Get(expelled[i].GetOwner())->GetNewArmy(CAUSE_NEW_ARMY_EXPELLED);
				} else {
							...
							sint32 &uindex) const
{
	...
	if ( !player_Get(m_owner)->m_advances->HasAdvance(advanceutil_GetNukeAdvance()))
		return false;
...
void ArmyData::SetPositionAndFixActors(const MapPoint &p)
{
	...
	for(sint32 i = 0; i < m_nElements; i++)
	{
		world_Get()->RemoveUnitReference(opos, m_array[i]);
...
//            : rand_ptr()
//            : gevmanager_Get()
//            : player array        : Player array [see Player::InitPlayer for
...
ORDER_RESULT ArmyData::SlaveRaid(const MapPoint &point)
{
	...
	else
	{
		...
		if(civrand().Next(100) < sint32(death * 100.0))
		{
			...
			player_Get(m_array[uindex].GetOwner())->
                                   ...
                                   Unit &home_city)
{
	...
	return player_Get(m_array[0].GetOwner())->GetSlaveCity(slpos, home_city);
}
...
ORDER_RESULT ArmyData::EnslaveSettler(const MapPoint &point, const sint32 uindex,
                              Unit home_city)
{
	sint32 r = player_Get(m_owner)->GetSlaveCity(m_pos, home_city);
	if(!r)
		return ORDER_RESULT_ILLEGAL;
...
ORDER_RESULT ArmyData::EstablishEmbassy(const MapPoint &point)
{
	...
	if(player_Get(m_owner)->HasWarWith(c.GetOwner()))
		return ORDER_RESULT_ILLEGAL;
...
ORDER_RESULT ArmyData::Launch(Order *order)
{
	...
	Unit destCity = world_Get()->GetCity(order->m_point);
...
ORDER_RESULT ArmyData::Target(Order *order)
{
	...
	Unit targetCity = world_Get()->GetCity(order->m_point);
...
void ArmyData::Reenter()
{
	...
	sint32 i;
	Unit city = world_Get()->GetCity(m_reentryPos);
	...
	else if(world_Get()->GetCell(m_reentryPos)->GetNumUnits() > (k_MAX_ARMY_SIZE - m_nElements))
	{
		return;
	}
	else
	{
		...
		for (i = 0; i < m_nElements; i++)
		{
			...
			world_Get()->InsertUnit(m_reentryPos, m_array[i], revealedUnits);
...
void ArmyData::ThisMeansWAR(PLAYER_INDEX defense_owner)
{
	...
	if(player_Get(m_owner)) {
		player_Get(m_owner)->ThisMeansWAR(defense_owner);
	}
}
...
//	bool CanPillageTileImp = true;
	...
	//			sint32 rushmod = g_theGovernmentDB->Get(player_Get(m_owner)->m_government_type)->GetUnitRushModifier();
	//			sint32 impgold = cell->AccessImprovement(i).GetMaterialCost();
	for(sint32 j = 0; j < m_nElements; j++) {
		sint32 rushmod = static_cast<sint32>(g_theGovernmentDB->Get(player_Get(m_owner)->m_government_type)->GetWonderRushModifier());
		sint32 goldcost = (cell->GetGoldProduced() * cell->GetNumDBImprovements()) * rushmod;
		if((m_array[j].GetDBRec()->GetCanCaptureTile()) && (player_Get(m_owner)->m_gold->GetLevel() > goldcost)){ // && (terrainutil_CanBeCaptured(m_pos))) {
			if (cellOwner != m_owner) {
				cell->SetOwner(m_owner);
				world_Get()->ChangeOwner(pos, cellOwner, m_owner);
				player_Get(cellOwner)->m_gold->AddGold(goldcost); //may crash because of barbs?
				player_Get(m_owner)->m_gold->SubGold(goldcost);
				return ORDER_RESULT_SUCCEEDED;
			}
		}
	}
...
//
// Additions by PFT Dec 18, 2004. And modified by Martin G�hmann.
//
//----------------------------------------------------------------------------
...
bool ArmyData::CanBombard(const MapPoint &point) const
{
	...
	world_Get()->GetArmy(point, defender);
...
//
// Globals    : player array            : Player array [see Player::InitPlayer for
//                                    initialized player data]
//
// Returns    : bool
//
...
{
	Unit c = world_Get()->GetCell(point)->GetCity();
	sint32 prob;
	...
	if(c.m_id != 0) {
		...
		for(i = 0; i < m_nElements; i++) {
			...
			////////////////////////
			bool Punstrike = true;
			if((!wonderutil_GetEnablesPunativeAirstrikes(player_Get(m_owner)->m_builtWonders)) && (!m_array[i].GetMovementTypeAir())){ //(!m_array[i].GetDBRec()->GetMovementTypeAir())){
				Punstrike = false;
			}
			// END EMOD
...
// Globals    : render_observer         : display manager
//              g_network               : multiplayer manager
//              player array                : list of active players
						...
						if(trec->GetCantPillage() == 0)
						{
							for(i = 0; i < m_nElements; i++)
							{
								if(!m_array[i].GetDBRec()->GetSneakBombard())
								{
									...
									defending_diplomat.LogViolationEvent(m_owner, PROPOSAL_TREATY_CEASEFIRE);
									world_Get()->CutImprovements(point);
...
//				}
	...
	bool Punstrike = true;
	if(!wonderutil_GetEnablesPunativeAirstrikes(player_Get(m_owner)->m_builtWonders)
	...
	for (sint32 i = m_nElements - 1; i>= 0; i--)
	{
			...
			m_array[i].CanPerformSpecialAction())
		{
			...
			else
			{
				...
				if (player_Get(player_view::VisiblePlayer())->IsVisible(m_pos))
				{
					if(player_view::IsAutoCenterOn())
					{
						 ...
						 sint32 argument, GAME_EVENT passedEvent)
{
	...
	if(network_Get().IsActive() && network_Get().IsLocalPlayer(m_owner) &&
	...
	{
		if(network_Get().IsActive() && order == UNIT_ORDER_ADD_EVENT) {
	...
	if(order == UNIT_ORDER_VICTORY_MOVE) {
		...
		if(execute &&
		   ...
		   player_Get(m_owner)) {
			...
			if(m_owner == player_view::CurPlayer() ||
...
bool ArmyData::ExecuteOrders(bool propagate)
{
	...
	if(m_orders->GetHead()->m_order != UNIT_ORDER_EXPEL_TO) {
		...
		if(network_Get().IsActive() && m_owner == player_view::VisiblePlayer() &&
		   !network_Get().IsMyTurn()) {
		   return false;
		}
	}
...
//		UNIT_ORDER_TYPE type = m_orders->GetHead()->m_order;
	...
	player_view::GetTopCurItem(player, id, type);
	if (Army(id) == me)
	{
		player_view::Refresh();
	}
...
{
	const Unit city = world_Get()->GetCity(pos);
...
bool ArmyData::ExecuteMoveOrder(Order *order)
{
	...
	if(NOWHERE == d)
	{
		...
		if (network_Get().IsClient())
		{
			network_Get().RequestResync(RESYNC_ARMY_POS);
		}
		...
		if(m_array[i].GetCargoCapacity() > 0) {
			for (int j = cell->UnitArmy()->Num() - 1; j >= 0; j--) {
				...
				if(m_array[i].CanCarry(u)) {
					...
					world_Get()->RemoveUnitReference(m_pos, u);
					u.UndoVision();
					u.RemoveUnitVision();
					m_array[i].InsertCargo(u);
					player_Get(m_owner)->RegisterInsertCargo
...
bool ArmyData::Move(WORLD_DIRECTION d, Order *order)
{
	...
	if(IsMovePointsEnough(newPos)) {
		if(IsOccupiedByForeigner(newPos)) {
			...
			{
				player_Get(m_owner)->ContactMade(defender->GetOwner());
			...
			if(player_Get(m_owner)->IsRobot())
			{
				...
				if(player_Get(m_owner)->WillViolateCeaseFire(defOwner) ||
				   player_Get(m_owner)->WillViolatePact(defOwner)) {
					sint32 oldAlliance = player_Get(m_owner)->m_broken_alliances_and_cease_fires;
					player_Get(m_owner)->BreakAlliance(defOwner);
					player_Get(m_owner)->BreakCeaseFire(defOwner, oldAlliance == player_Get(m_owner)->m_broken_alliances_and_cease_fires);
				}
			}
...
bool ArmyData::FinishMove(WORLD_DIRECTION d, MapPoint &newPos, UNIT_ORDER_TYPE order)
{
	...
	if
	  (
	      world_Get()->GetCity(newPos).m_id == 0
	...
	else
	{
		...
		if(didMove)
		{
			for(sint32 i = m_nElements - 1; i >= 0; i--)
			{
				...
				if(!world_Get()->CanEnter(newPos, moveType))
				{
					// That's clearly wrong
					if((moveType & k_BIT_MOVEMENT_TYPE_SHALLOW_WATER)
					&&!(moveType & k_BIT_MOVEMENT_TYPE_WATER)
					&&  world_Get()->GetCell(newPos)->GetEnv() & k_BIT_MOVEMENT_TYPE_WATER
...
bool ArmyData::CheckSpecialUnitMove(const MapPoint &pos)
{
		...
		if(city.GetOwner() != m_owner) {
			for(sint32 i = 0; i < m_nElements; i++) {
				...
				if(m_array[i].GetDBRec()->HasNuclearAttack()) {
					...
					{
						player_Get(m_owner)->AdjustEventPollution(pollution);
					}
	...
	CellUnitList *defender = world_Get()->GetCell(pos)->UnitArmy();
...
//		!player_Get(m_owner)->WillViolateCeaseFire(defense_owner) &&
//		!player_Get(m_owner)->WillViolatePact(defense_owner) &&
	if(
	           player_Get(attack_owner)->HasWarWith(defense_owner)
	   ||
	      (
	           player_Get(attack_owner)->IsRobot()
	...
	else
	{
		...
		Unit ta = GetTopVisibleUnit(player_view::VisiblePlayer());
		...
		Unit td = defender.GetTopVisibleUnit(player_view::VisiblePlayer());
...
//
// Description: Returns true if this army's owner (player_Get(m_owner))
...
bool ArmyData::VerifyAttack(UNIT_ORDER_TYPE order, const MapPoint &pos,
							sint32 defense_owner)
{
	if (player_Get(m_owner)->HasWarWith(defense_owner))
	{
		return true;
	}
	...
	if (    network_Get().IsActive()
	     ...
	     && (player_Get(m_owner)->m_networkGroup ==
	            player_Get(defense_owner)->m_networkGroup
	        )
	   )
	{
	...
	// if you have ParliamentaryVoteChance govt
	         ...
	         g_theGovernmentDB->Get(player_Get(m_owner)->m_government_type)
...
void ArmyData::RevealZOCUnits(const MapPoint &pos)
{
	...
	world_Get()->GetAdjacentUnits(units, pos);
	MapPoint hisPos;
...
void ArmyData::MoveUnits(const MapPoint &pos)
{
	...
	if(HasLeftMap())
	{
		player_view::ArmyRemoved(m_owner, Army(m_id));
	}
...
{
	if(!world_Get()->GetEmptyTransports(pos, transports)) // Make a list of the transports with empty slots at pos
		return false;
	...
	for (sint32 i = m_nElements - 1; i>= 0; i--)
	{
		for (sint32 j=0; j < transports.Num(); j++)
		{
				...
				transports[j].GetCargoCapacity() > 0)
			{
				...
				{
					network_Get().Block(m_owner);
					network_Get().Enqueue(transports[j].AccessData());
					network_Get().Unblock(m_owner);
				}
...
bool ArmyData::ExecuteUnloadOrder(Order *order)
{
	...
	CellUnitList *a = world_Get()->GetArmyPtr(to_pt);
	if(a && (0 < a->Num()))
	{
...
//		sint32 n = debark.Num();
		...
		} else if(world_Get()->GetCity(to_pt).m_id != 0 &&
			      world_Get()->GetCity(to_pt).GetOwner() != m_owner) {
...
{
	sint32 i;
	double cost = world_Get()->GetMoveCost(pos);   //GetEnvBase()->GetMovement() non imps for Denyenemy
	...
	for(i = m_nElements - 1; i >= 0; i--)
	{
		...
		// movement like railroads 4-12-2006
			...
			const TerrainImprovementRecord *trec = g_theTerrainImprovementDB->Get(imp);
			if(player_Get(m_owner)->HasWarWith(CellOwner)
			&& trec->GetDeniedToEnemy())
			{  //i.e. RailRoads see Cell::CalcTerrainMoveCost?
				sint32 enemycost;
				world_Get()->GetTerrain(pos)->GetEnvBase()->GetMovement(enemycost);
				c = enemycost;
			}
			else
			{
...
//	sint32 n_start_attackers = m_nElements;
	...
	player_Get(attack_owner)->MakeConvertedCitiesUnhappy(defense_owner);
...
{
	CellUnitList *units = world_Get()->GetCell(m_pos)->UnitArmy();
...
{
	if (player_Get(m_owner)->m_all_armies->Num() < 2 &&
	    player_Get(m_owner)->m_all_cities->Num() < 1)
		return;
	...
	for(sint32 i = m_nElements - 1; i >= 0; i--)
	{
		...
		if (    (cellOwner != m_owner)
		     ...
		     && !player_Get(m_owner)->HasWarWith(cellOwner)
		   ...
		   )
		{
			...
			///       foreign units.
			player_Get(cellOwner)->CreateUnit
...
bool ArmyData::ExecuteSpecialOrder(Order *order, bool &keepGoing)
{
	...
	{
		if(player_Get(m_owner)->m_gold->GetLevel() < order_rec->GetGold())
		{
			sint32 visiblePlayer = player_view::VisiblePlayer();
			if
			  (
	...
	switch(result)
	{
		case ORDER_RESULT_FAILED:
		{
			...
			&& player_Get(player_view::VisiblePlayer())->IsVisible(order->m_point)
			){
				render_observer::AddCenterMap(m_pos);
			}
		...
		case ORDER_RESULT_SUCCEEDED_INCOMPLETE:
			...
			if (useDefaultSuccessSound)
			{
				if(player_view::IsAutoCenterOn()
				&&!render_observer::TileWillBeCompletelyVisible(order->m_point.x, order->m_point.y)
				&& player_Get(player_view::VisiblePlayer())->m_vision->IsVisible(order->m_point)
				){
					render_observer::AddCenterMap(m_pos);
				}
				...
				if(player_Get(player_view::VisiblePlayer())->IsVisible(order->m_point))
				{
					render_observer::AddSpecialEffect(order->m_point, spriteID, soundID);
				}
			}
...
{
	Unit c = world_Get()->GetCity(point);
	if (!c.IsValid())
		return false;

	Player * p = player_Get(c.GetOwner());
	if(!p)
		return false;
									...
									UNIT_ORDER_TYPE order_type)
{
	...
	Cell *cell = world_Get()->GetCell(newPos);

	if(cell->GetOwner() >= 0 && cell->GetOwner() != m_owner &&
	   cell->GetOwner() != world_Get()->GetCell(m_pos)->GetOwner()) {
		...
		if(atLeastOneNonSpecialUnit) {
			Agreement ag = player_Get(cell->GetOwner())->FindAgreement(AGREEMENT_TYPE_DEMAND_LEAVE_OUR_LANDS, m_owner);
...
// Parameters : order_rec   : the order to test
//
// Globals    : player array    : player (capabilities)
//
...
{
	if (order_rec->GetUnitPretest_CanPlantNuke() &&
		!player_Get(m_owner)->m_advances->HasAdvance(advanceutil_GetNukeAdvance())
	   )
	{
		return false;
	}
...
//
// Globals    : player array	: player array [see Player::InitPlayer for initialized player data]
//
...
{
	Assert(player_Get(m_owner));
	bool        can_afford      = (player_Get(m_owner)->GetGold() >= order_rec->GetGold());
	...
	if(order_rec->GetTargetPretestAttackPosition() && (target_valid || cur_pos_valid))
	{
		...
		world_Get()->GetArmy(pos,targetArmy);

		if(world_Get()->GetCell(pos)->GetCity().IsValid()
		&& world_Get()->GetCell(pos)->GetCity().GetOwner() != m_owner
		){
			// Nothing
		}
	...
	{
		if (world_Get()->GetOwner(pos) != m_owner)
		{
			target_valid  = false;
			cur_pos_valid = false;
		}
...
bool ArmyData::TargetValidForOrder(const OrderRecord * order_rec, const MapPoint &pos)
{
	...
	Unit target_city = world_Get()->GetCity(pos);
	...
	world_Get()->GetArmy(pos, target_army);
	...
	case k_Order_TargetPretest_TradeRoute_Bit:
		target_valid |= ( world_Get()->GetCell(pos)->GetNumTradeRoutes() > 0);
		break;
	case k_Order_TargetPretest_TerrainImprovement_Bit:
		target_valid |= ( world_Get()->GetCell(pos)->GetNumDBImprovements() > 0);
		break;
...
void ArmyData::AssociateEventsWithOrdersDB()
{
	...
	for (sint32 order_index = 0; order_index < g_theOrderDB->NumRecords(); order_index++)
	{
		...
		{
			s_orderDBToEventMap[order_index] = GameEventManager::GetEventIndex(event_name);
		}
		else
		{
...
bool ArmyData::IsObsolete() const
{
	...
	for (sint32 i = 0; i < m_nElements; i++)
	{
		...
		{
			if (player_Get(m_owner)->HasAdvance(rec->GetObsoleteAdvance(a)->GetIndex()))
			{
				return true;
			}
		}
...
bool ArmyData::IsInVisionRangeAndCanEnter(MapPoint &pos) const
{
	...
	if(!world_Get()->IsOnSameContinent(pos, here))
		return false;
...
bool ArmyData::UpgradeTypeAndCosts(bool & full, sint32 & costs, sint32 & fullCosts, sint8 & numUpgrade, sint8 & numUpgradeAll) const
{
	...
	for(sint32 i = 0; i < m_nElements; ++i)
	{
		...
		if(type >= 0)
		{
			...
			if(costs + unit_costs <= player_Get(m_owner)->m_gold->GetLevel())
			{
				costs += unit_costs;
				++numUpgrade;
			}
...
void ArmyData::CheckHostileTerrain()
{
	...
	TerrainRecord const * trec = g_theTerrainDB->Get(world_Get()->GetTerrainType(m_pos));
	sint32 hpcost;
...
void ArmyData::CheckMineField()
{
	...
	if(terrainutil_HasMinefield(m_pos)
		...
		for(sint32 ti = 0; ti < world_Get()->GetCell(m_pos)->GetNumDBImprovements(); ++ti){ /// @todo use i instead of ti
			const TerrainImprovementRecord * tirec = g_theTerrainImprovementDB->Get(world_Get()->GetCell(m_pos)->GetDBImprovement(ti));
...
void ArmyData::BarbarianSpawning()
{
	...
	//Cell *ecell = world_Get()->GetCell(epos);
...
//				Cell *cell = world_Get()->GetCell(m_pos);
				...
				&& m_owner == PLAYER_INDEX_VANDALS
					...
					for(sint32 j = 0; j < drec->GetNumBarbarianCamps(); j++) {
						player_Get(m_owner)->CreateSpecialImprovement(drec->GetBarbarianCampsIndex(j), m_pos, 0);
					}
				}
			}
		}
	...
	if(m_owner == PLAYER_INDEX_VANDALS) {
		...
		sint32 barbhorde = player_Get(PLAYER_INDEX_VANDALS)->m_all_units->Num();
