...
public:
	...
	int GetEventCount();
public:
	// Held by value: it exists for the tracker's whole lifetime and is
	// never replaced, so it can never be null.
...
inline void to_json(nlohmann::json &j, EventTracker const &t)
{
	...
	PointerList<EventData>::Walker w(const_cast<PointerList<EventData> *>(&t.m_dataList));
	while (w.IsValid())
	{
