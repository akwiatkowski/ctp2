...
//   in a database, addion by Martin G�hmann.
...
//Added by Martin G�hmann to get the number of records in a database via slic
...
public:
	...
	sint32 GetValue(sint32 index, const char *valname) override {
		...
		Assert(rec);
		if(!rec)return 0; //Added by Martin G�hmann to avoid an access violation
		sint32 i;
