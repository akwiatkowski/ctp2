...
//
// - GetNearestWater function fixed by Martin G�hmann November 2nd 2003.
// - New Slic functions of CTP2.1 readded by Martin G�hmann and JJB.
...
//   advance strings by Martin G�hmann.
// - StringCompare function overloaded to allow the comparision between strings
//   string IDs and strings retrieved from builtins, by Martin G�hmann.
...
//   represents an invalid unit type. - Feb. 24th 2005 Martin G�hmann
...
// - Removed the old endgame database. (Aug 29th 2005 Martin G�hmann)
...
GameEventArgList *SlicArgList::CreateGameEventArgs(GAME_EVENT ev)
{
	...
	Assert(m_argValue.size() == gevmanager_Get()->GetNumArgs(ev));
...
SFN_ERROR Slic_HasAdvance::Call(SlicArgList *args)
{
	...
	//Added by Martin G�hmann to overload this function to allow also advance
...
SFN_ERROR Slic_AddCity::Call(SlicArgList *args)
{
	...
	slicengine_Get()->GetContext()->AddCity(city);
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_Kill::Call(SlicArgList *args)
{
	...
	Message message;
	slicengine_Get()->GetCurrentMessage(message);

	if(message == Message())
		return SFN_ERROR_NOT_IN_BUTTON;

	slicengine_Get()->KillCurrentMessage();
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_MinimizeAction::Call(SlicArgList *args)
{
	...
	slicengine_Get()->AddCurrentMessage();
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_DeactivateTutorial::Call(SlicArgList *args)
{
	...
	slicengine_Get()->SetTutorialActive(FALSE);
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_MessageType::Call(SlicArgList *args)
{
	...
	slicengine_Get()->GetContext()->SetMessageType((MESSAGE_TYPE)msgTypeIndex,
...
SFN_ERROR Slic_Duration::Call(SlicArgList *args)
{
	...
	slicengine_Get()->GetContext()->SetMessageDuration(duration);
...
SFN_ERROR Slic_AddOrder::Call(SlicArgList *args)
{
	...
	if(slicengine_Get()->GetContext()->GetNumOrders() > 0) {
		order = slicengine_Get()->GetContext()->GetOrder(0);
	}
...
SFN_ERROR Slic_Abort::Call(SlicArgList *args)
{
	if(args->Count() > 0)
		return SFN_ERROR_NUM_ARGS;
	slicengine_Get()->GetContext()->Abort();
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_Show::Call(SlicArgList *args)
{
	if(args->Count() > 0)
		return SFN_ERROR_NUM_ARGS;
	slicengine_Get()->GetContext()->SetInstant();
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_MessageClass::Call(SlicArgList *args)
{
	...
	slicengine_Get()->GetContext()->SetClass(type);
...
SFN_ERROR Slic_DontSave::Call(SlicArgList *args)
{
	...
	slicengine_Get()->GetContext()->SetDontSave();
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_DisableClose::Call(SlicArgList *args)
{
	...
	slicengine_Get()->GetContext()->DisableClose();
...
SFN_ERROR Slic_AddGoods::Call(SlicArgList *args)
{
	...
	city.AddGoods(slicengine_Get()->GetContext());
	return SFN_ERROR_OK;
}
...
{
	return slicengine_Get()->GetFunction("GoodCount")->Call(args);
}
...
SFN_ERROR Slic_StartTimer::Call(SlicArgList *args)
{
	...
	slicengine_Get()->StartTimer(timer, duration);
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_StopTimer::Call(SlicArgList *args)
{
	...
	slicengine_Get()->StopTimer(timer);
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_DisableMessageClass::Call(SlicArgList *args)
{
	...
	slicengine_Get()->DisableMessageClass(msgClass);
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_EnableMessageClass::Call(SlicArgList *args)
{
	...
	if(!args->GetInt(0, msgClass))
		return SFN_ERROR_TYPE_ARGS;
	slicengine_Get()->EnableMessageClass(msgClass);
...
//
// Globals    : gevmanager_Get()	: game event handler
//
...
SFN_ERROR Slic_AddUnit::Call(SlicArgList *args)
{
	...
	slicengine_Get()->GetContext()->AddUnit(u);
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_GetNearestWater::Call(SlicArgList *args)
{
	...
	SlicSymbolData *sym = args->m_argValue[1].m_symbol;
	//Outcommented by Martin G�hmann
...
SFN_ERROR Slic_LoadGame::Call(SlicArgList *args)
{
	...
	if(!args->GetString(0, string))
		return SFN_ERROR_TYPE_ARGS;
	slicengine_Get()->SetLoadGame(string);
...
SFN_ERROR Slic_SetTimerGranularity::Call(SlicArgList *args)
{
	...
	slicengine_Get()->SetTimerGranularity(gran);
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_KillEyepointMessage::Call(SlicArgList *args)
{
	...
	if(messagepool_Get()->IsValid(slicengine_Get()->GetEyepointMessage())) {
		slicengine_Get()->GetEyepointMessage().Kill();
	}
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_UseDirector::Call(SlicArgList *args)
{
	...
	slicengine_Get()->GetContext()->SetUseDirector();
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_SetUnit::Call(SlicArgList *args)
{
	...
	slicengine_Get()->GetContext()->SetUnit(index, u);
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_SetCity::Call(SlicArgList *args)
{
	...
	slicengine_Get()->GetContext()->SetCity(index, c);
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_SetLocation::Call(SlicArgList *args)
{
	...
	slicengine_Get()->GetContext()->SetLocation(index, point);
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_SetOrder::Call(SlicArgList *args)
{
	...
	slicengine_Get()->GetContext()->SetOrder(index, (UNIT_ORDER_TYPE)order);
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_SetActionKey::Call(SlicArgList *args)
{
	...
	if(keystring[0] != '^') {
		slicengine_Get()->SetTriggerKey(index - 1, keystring[0]);
	} else {
		slicengine_Get()->SetTriggerKey(index - 1, keystring[1] + 128);
	}
	return SFN_ERROR_OK;
}
...
SFN_ERROR Slic_SetString::Call(SlicArgList *args)
{
	...
	slicengine_Get()->GetContext()->SetAction(index, str);
	return SFN_ERROR_OK;
}
...
{
	//Wrong number of arguments added by Martin G�hmann
	if(args->Count() != 0)
		return SFN_ERROR_NUM_ARGS;
...
//New Slic functions of CTP2.1 readded by Martin G�hmann
...
SFN_ERROR Slic_CityHasWonder::Call(SlicArgList *args)
{
	...
	if (wonder >= 0 && wonder < 64 &&
		(city.GetCityData()->GetBuiltWonders() & ((uint64)1 << wonder)))
    {
		m_result.m_int = 1;
	}
    else
