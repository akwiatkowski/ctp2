...
char const *HEADLESS_CANDIDATES[] = {
    "./build/ctp2_headless",
...
char const *find_headless_binary()
{
    for (char const **p = HEADLESS_CANDIDATES; *p; ++p)
    {
...
{
    char const *bin = find_headless_binary();
    if (!bin) { out = "[ERROR] ctp2_headless not found"; return -1; }
