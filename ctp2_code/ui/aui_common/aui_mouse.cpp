		...
		if ( numCursors )
		{
			...
			for ( sint32 i = 0; i < numCursors; i++ )
			{
				...
				if ( filename )
				{
					...
					{
						aui_ui_Get()->UnloadCursor( cursor );
...
void aui_Mouse::SetClip( sint32 left, sint32 top, sint32 right, sint32 bottom )
{
	if ( left < 0 ) left = 0;
	if ( top < 0 ) top = 0;
	if ( right > aui_ui_Get()->Width() ) right = aui_ui_Get()->Width();
	if ( bottom > aui_ui_Get()->Height() ) bottom = aui_ui_Get()->Height();
...
void aui_Mouse::SetClip( RECT *clip )
{
	if ( clip )
		...
		SetClip( 0, 0, aui_ui_Get()->Width(), aui_ui_Get()->Height() );
}
...
void aui_Mouse::SetCursor(sint32 index, MBCHAR * cursor)
{
	...
	if (oldCursor) aui_ui_Get()->UnloadCursor(oldCursor);
}
...
AUI_ERRCODE	aui_Mouse::BltWindowToPrimary( aui_Window *window )
{
	...
	for ( sint32 j = windowDirtyList->L(); j; j-- )
	{
		...
		if ( f )
		{
			aui_ui_Get()->ColorBltToSecondary(
				&screenDirtyRect,
				RGB(255,255,255),
				0 );
#ifdef WIN32
...
#ifdef SEIZUREBLIT
		{
			aui_ui_Get()->TheBlitter()->ColorBlt(
				aui_ui_Get()->Secondary(),
				&screenDirtyRect,
				RGB(255,255,255),
				0 );
