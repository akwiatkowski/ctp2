...
public:
	...
	) override
    {
        if (m_window)
        {
            std::unique_ptr<aui_Window> window(m_window);
            std::unique_ptr<aui_Region>{window->GetChildByIndex(0)};
        };
    };
...
ctp2_Menu::~ctp2_Menu()
{
	...
	if(m_window) {
		...
		c3ui_Get()->AddAction(std::make_unique<CleanupMenuWindowAction>(m_window).release());
	}

	if(m_items) {
		if(!m_resized) {

			PointerList<Item>::Walker walk(m_items.get());
			for(; walk.IsValid(); walk.Next()) {
				std::unique_ptr<ctp2_ListItem>{walk.GetObj()->m_item};
			}
		}
...
void ctp2_Menu::Init(const MBCHAR *block, bool atMouse, CTP2MenuCallback *callback)
{
	m_window = nullptr;
	m_list = nullptr;
	m_items = std::make_unique<PointerList<Item>>();
									   ...
									   const MBCHAR *icon, void *cookie)
{
	...
	std::unique_ptr<ctp2_Menu::Item> menuItem = std::make_unique<ctp2_Menu::Item>();
	...
	ctp2_Menu::Item *menuItemRaw = menuItem.release();
	m_items->AddTail(menuItemRaw);
...
{
	ctp2_Menu::Item *item = CreateItem(const_cast<MBCHAR *>("PlainMenuListItem"), text, shortcut, nullptr, cookie);
	Assert(item);
}

void ctp2_Menu::AddItemWithIcon(const MBCHAR *text, const MBCHAR *icon, const MBCHAR *shortcut, void *cookie)
{
	ctp2_Menu::Item *item = CreateItem(const_cast<MBCHAR *>("IconMenuListItem"), text, shortcut, icon, cookie);
