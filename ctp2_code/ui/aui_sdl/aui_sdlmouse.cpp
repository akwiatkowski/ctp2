...
aui_SDLMouse::GetInput()
{
	...
	for (sint32 numInputs = 200; numInputs; numInputs--)
	{
		...
		int numElements = SDL_PeepEvents(&od, 1, SDL_GETEVENT, SDL_MOUSEMOTION, SDL_MOUSEBUTTONUP);
		if (0 > numElements)
		{
		...
		case SDL_MOUSEMOTION:
		{
			...
			if (++motionLogCount <= 20)
			{
				fprintf(stderr, "[MOUSE-IN] SDL motion: (%d, %d) state=%d\n",
				        static_cast<int>(od.motion.x), static_cast<int>(od.motion.y),
				        od.motion.state);
			}
			break;
		...
		static uint32 lastRecordedMotion = 0;
		bool const isMotion = od.type == SDL_MOUSEMOTION;
		if (!isMotion || m_data.time - lastRecordedMotion >= 50)
		{
			if (isMotion)
				lastRecordedMotion = m_data.time;
			playtest_recorder::RecordInput(
			    isMotion                         ? "mouse_motion"
			    : od.type == SDL_MOUSEBUTTONDOWN ? "mouse_down"
			                                     : "mouse_up",
			    m_data.time, m_data.position.x, m_data.position.y,
			    isMotion ? static_cast<std::int32_t>(m_data.lbutton)
			             : static_cast<std::int32_t>(od.button.button),
			    isMotion ? static_cast<std::int32_t>(m_data.rbutton)
			             : static_cast<std::int32_t>(CTP2_SDL_IsMouseButtonDown(od)),
			    !isMotion);
		}
		if (!isMotion)
			return AUI_ERRCODE_OK;
	...
	return haveMoves ? AUI_ERRCODE_OK : AUI_ERRCODE_NOINPUT;
}
