...
//   the player to optimize sliders, automaticly. - April 8th 2005 Martin G�hmann
...
//   is moved. Jul 7th 2005 Martin G�hmann
...
m_optimizeSliderButton(static_cast<ctp2_Button*>(aui_Ldl::GetObject(
	"DomesticDialog.OptimizeSlidersButton"))), // Added by Martin G�hmann
...
void CauseAndEffectTab::UpdateCommerceValues()
{
	...
	for(j = 0; j < k_MAX_PLAYERS; j++) {
		...
		for(k = player_Get(j)->m_all_cities->Num() - 1; k >= 0; k--) {
			if (player_Get(j)->m_all_cities->Access(k)->GetCityData()->IsConvertedTo() ==
				player->GetOwner())
				totalGoldConversions +=
					player_Get(j)->m_all_cities->Access(k)->GetCityData()->GetConvertedGold();
		}
	}
