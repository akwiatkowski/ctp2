...
//   or infrastructure, by Martin G�hmann.
// - No turn numbers aren't shown anymore in the listbox if the build item is
//   infrastructure or capitalization, by Martin G�hmann.
...
//   critical. - April 6th 2005 Martin G�hmann
// - Added City Manager button and functions callback. - July 24th 2005 Martin G�hmann
// - National Manager window will always be displayed on top of other
//   windows when activated. - July 24th 2005 Martin G�hmann
...
// cast in one place; a null item yields an invalid Unit.
static Unit CityFromListItem(ctp2_ListItem *item)
{
    Unit city;
    if (item)
...
static void StoreCityInListItem(ctp2_ListItem *item, Unit city)
{
    item->SetUserData(reinterpret_cast<void*>(static_cast<uintptr_t>(city.m_id)));
}
												  ...
												  sint32 column)
{
	...
	Unit city1 = CityFromListItem(item1);
	Unit city2 = CityFromListItem(item2);
...
void NationalManagementDialog::DisbandCallback(bool response, void *userData)
{
	...
	if(response) {
		...
		for(uint32 selectIndex = 0; selectIndex < selectedList->L(); selectIndex++) {
			...
			Unit city = CityFromListItem(item);
