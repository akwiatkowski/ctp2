...
// - Added a civ city style choser on the civ tab. (Jan 4th 2005 Martin G�hmann)
// - Spinner callbacks are added at the end so that they aren't called due to
//   min or max modifications. This accelerates Scenario Editor initalisation. (Feb 4th 2007 Martin G�hmann)
// - Switching between players now updates the city list of the main control
//   panel city tab. (Feb 4th 2007 Martin G�hmann)
...
//	ctp2_Button *m_tabButton[k_NUM_TAB_BUTTONS];
    ...
    m_cityStyle                 (CITY_STYLE_EDITOR), 	//displayed in the CityPopSpinner to new created cities.
 	m_newPopSize                (1), 	//Added by Martin G�hmann to add the pop number
	...
	// MBCHAR m_scenarioName[k_SCENARIO_NAME_MAX];
{
	...
	for(i = 0; i < k_NUM_PLAYER_SPINNERS; i++)
	{
		...
		if(spin)
		{
			//Added by Martin G�hmann to make sure that the Scenario Editor
			//does not set the player to player 1 when the scenario editor
			//is loaded for the first time in a session.
			spin->SetValue((sint32)selitem_Get()->GetPlayerOnScreen(), 0);
			spin->SetMinimum(0, 0);
	...
	if(spin)
	{
		...
		sint32 style = player_Get(selitem_Get()->GetPlayerOnScreen())->GetCivilisation()->GetCityStyle();
...
void ScenarioEditor::Reupdate()
{
	...
	if(player_Get(selitem_Get()->GetVisiblePlayer())) {
		player_Get(selitem_Get()->GetVisiblePlayer())->m_playerType = PLAYER_TYPE_HUMAN;
		NewTurnCount::SetStopPlayer(selitem_Get()->GetVisiblePlayer());
	}
...
//Added by Martin G�hmann to show the according city style name in the tooltip.
...
//Added by Martin G�hmann to make
...
void ScenarioEditor::CityPopSpinner(aui_Control *control, uint32 action, uint32 data, void *cookie)
{
	...
	//Added by Martin G�hmann to make shure
...
void ScenarioEditor::CivCityStyleSpinner(aui_Control *control, uint32 action, uint32 data, void *cookie)
{
	...
	if(rec){
		player_Get(selitem_Get()->GetVisiblePlayer())->GetCivilisation()->AccessData()->SetCityStyle(spinner->GetValueX());
...
void ScenarioEditor::CivAddRemovePlayer(aui_Control *control, uint32 action, uint32 data, void *cookie)
{
	...
	if(s_scenarioEditor) {
		...
		if(player_Get(pl)) {
			...
			selitem_Get()->SetPlayerOnScreen(pl);
			selitem_Get()->SetCurPlayer(pl);
...
void ScenarioEditor::NotifyPlayerChange()
{
	...
	if(player_Get(player)) {
		...
		ctp2_DropDown *govs = (ctp2_DropDown *)aui_Ldl::GetObject(s_scenarioEditorBlock, "TabGroup.Civ.SetGovernment");
		govs->SetSelectedItem(player_Get(selitem_Get()->GetVisiblePlayer())->GetGovernmentType());
	}
}
...
void ScenarioEditor::PlayerSpinner(aui_Control *control, uint32 action, uint32 data, void *cookie)
{
	...
	if(player_Get(newPlayer)) {
		...
		selitem_Get()->SetPlayerOnScreen(newPlayer);
		selitem_Get()->SetCurPlayer(newPlayer);
		director_Get()->AddCopyVision();
...
void ScenarioEditor::PlaceFlag(MapPoint &pos)
{
	...
	sint32 playerOrCiv = selitem_Get()->GetVisiblePlayer();
	if(s_scenarioEditor->m_startLocMode == SCEN_START_LOC_MODE_CIV) {
...
void ScenarioEditor::Year(aui_Control *control, uint32 action, uint32 data, void *cookie)
{
	...
	PLAYER_INDEX current_player = selitem_Get()->GetCurPlayer();
	...
	MainControlPanel::UpdatePlayer(selitem_Get()->GetVisiblePlayer());
}
...
void ScenarioEditor::SetGovernment(aui_Control *control, uint32 action, uint32 data, void *cookie)
{
	...
	ctp2_DropDown *dd = (ctp2_DropDown *)control;
	player_Get(selitem_Get()->GetVisiblePlayer())->ActuallySetGovernment
	    (dd->GetSelectedItem());
}
...
//Added by Martin G�hmann to get the last player in the game.
